## 1000 и одна сумка

	name=1000 и одна сумка
	shop=bag
	website=https://1000i1sumka.ru/
	contact:vk=https://vk.com/sumok1001
	opening_hours=11:00-21:00

* можно проверить на карте и в списке https://1000i1sumka.ru/shops

## 1x

	name=1x
	shop=bookmaker
	website=https://1xstavka.ru/

* можно проверить на карте и в списке https://1xstavka.ru/points/

## 220 Вольт

	name=220 Вольт
	brand=220 Вольт
	brand:wikidata=Q18635304
	shop=power_tools
	website=http://www.220-volt.ru/
	contact:vk=https://vk.com/likevolt
	opening_hours=10:00-20:00

* `website` можно уточнить на карте и в списке http://www.220-volt.ru/shop/1/city-7800000000000/

## 36,6

	name=36,6
	brand=36,6
	brand:wikidata=Q226889
	amenity=pharmacy
	website=https://366.ru/
	contact:vk=https://vk.com/apteka36i6

* `website` можно уточнить на карте https://366.ru/apteki/map/

## 585 Gold

	name=585 Gold
	shop=jewelry
	website=https://zoloto585.ru/
	contact:vk=https://vk.com/zoloto585gold

* `website` можно уточнить на карте https://zoloto585.ru/about/address/

## Alex Fitness

	name=Alex Fitness
	leisure=fitness_centre
	website=https://alexfitness.ru/
	contact:vk=https://vk.com/alexfitness

* `website` можно уточнить на карте https://alexfitness.ru/#map
* `contact:vk` можно уточнить

## All Seasons Комфорт

	name=All Seasons Комфорт
	shop=clothes
	website=http://allseasonscomfort.ru/

Альтернативные теги:

	name=Комфорт Все сезоны

* можно проверить в списке http://allseasonscomfort.ru/shops/

## Aloha

	name=Aloha
	shop=travel_agency
	website=https://alohatur.ru/
	contact:vk=https://vk.com/alohatour

* `website` можно уточнить на карте https://alohatur.ru/contacts

## Anex Tour

	name=Anex Tour
	shop=travel_agency
	website=https://b2c.anextour.com/
	contact:vk=https://vk.com/anextourrussia

* разные операторы

## Artis

	name=Artis
	shop=frame
	website=https://artis-baget.ru/
	contact:vk=https://vk.com/club12287262

* можно проверить в списке https://artis-baget.ru/contacts/

## Baggins

	name=Baggins
	shop=bag
	website=https://baggins.ru/
	contact:vk=https://vk.com/bagginsru

Альтернативные теги:

	opening_hours=10:00-20:00
	opening_hours=Mo-Sa 10:00-20:00; Su 10:00-19:00
	opening_hours=10:00-21:00

* можно проверить на карте https://baggins.ru/contacts/

## Baggins Coffee

	name=Baggins Coffee
	amenity=cafe
	cuisine=coffee_shop
	website=http://bagginscoffee.ru/
	contact:vk=https://vk.com/bagginscoffee
	opening_hours=Mo-Sa 08:00-21:00; Su 09:00-21:00

Альтернативные теги:

	amenity=fast_food

## Beer House

	name=Beer House
	amenity=pub
	website=http://beerhousebar.ru/
	contact:vk=https://vk.com/beerhousebar
	opening_hours=12:00+
	operator=ООО «Юник»

Альтернативные теги:

	name=Beer House Staff

* можно проверить на карте и в списке http://beerhousebar.ru/

## Bengel

	name=Bengel
	shop=bakery
	website=http://bengel.ru/
	contact:vk=https://vk.com/bengelbakery

* можно проверить на карте и в списке http://bengel.ru/#maps

## Bileter

	name=Bileter
	shop=ticket
	tickets:theatre=yes
	website=https://www.bileter.ru/
	contact:vk=https://vk.com/bileter
	opening_hours=10:00-21:00; 15:00-16:00 off

* можно проверить на карте и в списке https://www.bileter.ru/ticket-offices

## Calzedonia

	name=Calzedonia
	brand=Calzedonia
	brand:wikidata=Q1027874
	shop=clothes
	clothes=underwear
	website=https://ru.calzedonia.com/
	contact:vk=https://vk.com/calzedoniaofficial

* `website` можно уточнить поиском на https://ru.calzedonia.com/custserv/locate_store.cmd

## Coffee Bon

	name=Coffee Bon
	amenity=fast_food
	cuisine=coffee_shop
	contact:vk=https://vk.com/coffee_bon

## Coffee Like

	name=Coffee Like
	amenity=fast_food
	cuisine=coffee_shop
	website=http://coffee-like.com/
	contact:vk=https://vk.com/coffeelike78

* можно проверить на карте http://coffee-like.com/coffee-like-v-tvoem-gorode

## Coffeers

	name=Coffeers
	amenity=fast_food
	cuisine=coffee_shop
	website=http://coffeers.ru/
	contact:vk=https://vk.com/coffeers

* можно проверить на карте внизу страницы http://coffeers.ru/

## Cofix

	name=Cofix
	brand=Cofix
	brand:wikidata=Q16132717
	amenity=fast_food
	cuisine=coffee_shop
	website=http://spb.cofix.ru/
	contact:vk=https://vk.com/cofixrussia
	operator=ООО «Кофе-фикс»

* можно проверить в списке и на карте http://spb.cofix.ru/cafe/

## Coral Travel

	name=Coral Travel
	shop=travel_agency
	website=http://www.coral.ru/
	contact:vk=https://vk.com/coraltravel
	opening_hours=Mo-Fr 10:00-21:00; Sa-Su 11:00-20:00
	operator=ООО «Аксиома»

* `website` можно уточнить на карте http://www.coral.ru/offices

## Cream.Shop

	name=Cream.Shop
	shop=cosmetics
	website=https://deadsea-cosmetics.ru/
	contact:vk=https://vk.com/crem.shop
	opening_hours=10:00-22:00
	operator=ООО «Крем шоп»

* `website` можно уточнить на карте и в списке https://deadsea-cosmetics.ru/shops/spb-shops/

## DNS

	name=DNS
	brand=DNS
	brand:wikidata=Q4036922
	shop=computer
	website=https://www.dns-shop.ru/
	contact:vk=https://vk.com/dnsstore
	opening_hours=10:00-21:00
	operator=ООО «Компьютерный Мир - На Сенной»

Альтернативные теги:

	name=DNS сервис
	name=DNS цифровой

	craft=electronics_repair

* `website` можно уточнить на карте и в списке https://www.dns-shop.ru/shops/spb/

## Duty Free Flowers

	name=Duty Free Flowers
	shop=florist
	website=http://dff24.ru/
	contact:vk=https://vk.com/dff24

Альтернативные теги:

	opening_hours=24/7
	opening_hours=Mo-Fr 07:00-22:00; Sa-Su 10:00-22:00

* можно проверить в списке http://dff24.ru/shops

## Ecco

	name=Ecco
	brand=Ecco
	brand:wikidata=Q1280255
	shop=shoes
	website=https://www.ecco-shoes.ru/
	contact:vk=https://vk.com/eccoru
	opening_hours=10:00-22:00

* `website` можно уточнить на карте https://www.ecco-shoes.ru/shops/

## Elit

	name=Elit
	shop=clothes
	clothes=workwear
	contact:vk=https://vk.com/spb_elit
	website=http://tdelit.ru/
	opening_hours=10:00-20:00

## English First

	name=English First
	brand=EF Education First
	brand:wikidata=Q661869
	amenity=language_school
	website=https://www.ef.ru/
	opening_hours=Mo-Fr 09:00-22:00; Sa 10:00-16:00; Su 10:00-15:00

Альтернативные теги:

	contact:vk=https://vk.com/englishfirstadults

* `website` (для взрослых) можно уточнить в списке и на карте https://www.ef.ru/englishfirst/adults/centers/st-petersburg/
* `website` (для детей) можно уточнить в списке и на карте https://www.ef.ru/englishfirst/kids/city/stpetersburg

## Ergo

	name=Ergo
	brand=Ergo
	brand:wikidata=Q541909
	office=insurance
	website=http://ergo.ru/
	contact:vk=https://vk.com/ergorussia
	opening_hours=Mo-Fr 10:00-19:00
	operator=САО Эрго

* можно проверить в списке http://ergo.ru/contacts/saint-petersburg/

## Etlon Coffee

	name=Etlon Coffee
	amenity=fast_food
	cuisine=coffee_shop
	contact:vk=https://vk.com/etloncoffee

* можно проверить в списке https://vk.com/etloncoffee?w=page-140109615_54648833

## Extaz

	name=Extaz
	shop=erotic
	website=https://www.extazshop.ru/
	contact:vk=https://vk.com/extazshop
	opening_hours=24/7

* `website` можно уточнить

## Feromon

	name=Feromon
	amenity=hookah_lounge
	website=http://feromon.group/
	contact:vk=https://vk.com/kalyannie_spb

* `website` и `contact:vk` можно уточнить в списке http://feromon.group/

## Fix Price

	name=Fix Price
	brand=Fix Price
	brand:wikidata=Q4038791
	shop=variety_store
	website=https://fix-price.ru/
	contact:vk=https://vk.com/fix_price
	operator=ООО «Бэст Прайс»

Альтернативные теги:

	opening_hours=09:00-21:00
	opening_hours=10:00-22:00

* можно проверить на карте и в списке https://fix-price.ru/buyers/shops/sankt-peterburg/

## Goldenpoint

	name=Goldenpoint
	contact:vk=https://vk.com/club23701382
	shop=clothes
	website=https://www.goldenpoint.com/
	opening_hours=09:00-21:00
	operator=ООО «Митинтертрейд»

* можно проверить поиском на https://www.goldenpoint.com/it_en/store-locator/

## Golfstream

	name=Golfstream
	shop=clothes
	clothes=underwear
	website=http://golfstream-shop.ru/
	contact:vk=https://vk.com/shop.golfstream
	opening_hours=10:00-21:00
	operator=ИП Дорофеева Ю.В.

* можно проверить на карте и в списке http://golfstream-shop.ru/stores/

## Greenbox

	name=Greenbox
	amenity=cafe
	website=http://greenboxes.ru/
	contact:vk=https://vk.com/greenbox_spb
	opening_hours=08:00-23:00

* можно проверить на карте и в списке http://greenboxes.ru/#Cont

## Högl

	name=Högl
	shop=shoes
	website=https://hoegl.ru/
	contact:vk=https://vk.com/hogl_russia
	opening_hours=10:00-21:00
	operator=ООО «Хёгель Шу Фэшн»

* можно проверить на карте и в списке https://hoegl.ru/about/stores/

## IL Патио

	name=IL Патио
	amenity=cafe
	website=http://spb.ilpatio.ru/
	contact:vk=https://vk.com/ilpatio_ru

* можно проверить на карте и в списке http://spb.ilpatio.ru/restaurants/

## Intimissimi

	name=Intimissimi
	brand=Intimissimi
	brand:wikidata=Q305404
	shop=clothes
	clothes=underwear
	website=https://ru.intimissimi.com/
	contact:vk=https://vk.com/intimissimiofficial
	opening_hours=10:00-22:00

* `website` можно уточнить поиском на https://ru.intimissimi.com/custserv/locate_store.cmd?cityStateZip=190000

## Jäger

	name=Jäger
	amenity=pub
	website=http://www.jagerhaus.ru/
	contact:vk=https://vk.com/jagerhaus
	old_name=Jager Haus
	operator=ООО «Егерь»

Альтернативные теги:

	opening_hours=Su-Th 11:30-01:00, Fr-Sa 11:30-06:00
	opening_hours=Su-Th 11:30-01:00, Fr-Sa 11:30-02:30

* `website` можно уточнить в списке http://www.jagerhaus.ru/#contacts

## K-9

	name=K-9
	shop=pet
	contact:vk=https://vk.com/zoo_k9
	opening_hours=10:00-20:00

* `contact:vk` можно уточнить для некоторых точек

## Kassir.ru

	name=Kassir.ru
	shop=ticket
	tickets:theatre=yes
	website=https://spb.kassir.ru/
	contact:vk=https://vk.com/spbkassir
	opening_hours=10:00-20:00; 14:00-15:00 off

* можно проверить на карте и в списке https://spb.kassir.ru/biletnye-kassy

## Kerama Marazzi

	name=Kerama Marazzi
	shop=tiles
	website=http://spb.kerama-marazzi.com/
	contact:vk=https://vk.com/keramamarazzi
	opening_hours=Mo-Sa 11:00-20:00; Su 11:00-17:00

Альтернативные теги:

	operator=ООО «Балткерама»
	operator=ООО «Керама-СПб»

* можно проверить на карте и в списке http://spb.kerama-marazzi.com/ru/wheretobuy/shops.php?IBLOCK_ID=13&SECTION_ID=1473

## KFC

	name=KFC
	brand=KFC
	brand:wikidata=Q524757
	amenity=fast_food
	website=https://www.kfc.ru/
	contact:vk=https://vk.com/kfcrussia
	operator=ООО «АмРест»

Альтернативные теги:

	opening_hours=Mo-Fr 08:00-22:00; Sa-Su 09:00-22:00
	opening_hours=07:00-06:00
	opening_hours=24/7

* `website` можно уточнить на карте https://www.kfc.ru/restaurants

## Kiss Fashion

	name=Kiss Fashion
	shop=bag
	website=http://kissfashion.ru/
	contact:vk=https://vk.com/club91432997
	opening_hours=10:00-21:00

* `website` можно уточнить в списке http://kissfashion.ru/kontakty
* можно проверить в списке https://vk.com/topic-91432997_32069710

## Kistochki

	name=Kistochki
	shop=beauty
	website=http://kistochki.ru/
	contact:vk=https://vk.com/kistochki_nails

Альтернативные теги:

	opening_hours=10:00-22:00
	opening_hours=24/7

* можно проверить в списке и на карте http://kistochki.ru/address/

## Kriek

	name=Kriek
	amenity=pub
	website=http://www.kriek.ru/
	contact:vk=https://vk.com/kriek
	opening_hours=Su-Th 11:00-01:00, Fr-Sa 11:00-02:30

* `website` можно уточнить в списке http://www.kriek.ru/#contacts

## Lady Sharm

	name=Lady Sharm
	shop=clothes
	clothes=women
	website=http://ledisharm.com/
	contact:vk=https://vk.com/ladysharmclassic
	opening_hours=Mo-Sa 10:00-21:00; Su 10:00-19:00
	operator=ООО «Камея»

* можно проверить в списке http://ledisharm.com/magazini-lady-sharm/
* можно проверить на карте http://ledisharm.com/magazini-lady-sharm/spb/

## Lamoda

	name=Lamoda
	shop=clothes
	reservation=required
	website=https://www.lamoda.ru/
	contact:vk=https://vk.com/lamodaru

* можно проверить на карте https://www.lamoda.ru/delivery/

## Lilkko

	name=Lilkko
	shop=gift
	website=http://www.lilkko.ru/
	contact:vk=https://vk.com/lilkko

* можно проверить в списке http://www.lilkko.ru/page/shops

## Lobby Coffee

	name=Lobby Coffee
	amenity=fast_food
	cuisine=coffee_shop
	website=http://lobbycoffee.ru/
	contact:vk=https://vk.com/lobbycoffeespb

## Love Zona

	name=Love Zona
	shop=erotic
	website=https://love-z.ru/
	contact:vk=https://vk.com/love.zona
	opening_hours=24/7

* `website` можно уточнить на карте и в списке https://love-z.ru/shops

## Lucky Fish

	name=Lucky Fish
	alt_name=Икра и рыба
	shop=seafood
	website=http://икраирыба.рф/
	contact:vk=https://vk.com/ikrairybaspb

* можно проверить на карте и в списке http://икраирыба.рф/adresa-magazinov/

## Lush

	name=Lush
	brand=Lush
	brand:wikidata=Q1585448
	shop=cosmetics
	website=https://www.lushrussia.ru/
	contact:vk=https://vk.com/lush_russia
	opening_hours=10:00-22:00
	operator=ООО «Лаш Раша»

* можно проверить на карте и в списке https://www.lushrussia.ru/about/shops/sankt_peterburg/

## Mama Roma

	name=Mama Roma
	amenity=restaurant
	cuisine=italian
	website=http://www.mamaroma.ru/
	contact:vk=https://vk.com/mamaroma_official
	opening_hours=09:00-01:00

* `website` можно уточнить в списке http://www.mamaroma.ru/web/ru/contacts/

## Marketplace

	name=Marketplace
	amenity=cafe
	website=http://market-place.me/ru/spb/
	contact:vk=https://vk.com/marketplacespb

## Marko

	name=Marko
	shop=shoes
	website=http://www.markoclub.ru/
	contact:vk=https://vk.com/marko_obuv

* можно проверить в списке http://www.markoclub.ru/belobuvshops

## mi-shop.com

	name=mi-shop.com
	brand=Xiaomi
	brand:wikidata=Q1636958
	shop=mobile_phone
	website=https://mi-shop.com/
	contact:vk=https://vk.com/mi.russia
	opening_hours=10:00-22:00

* можно проверить на карте и в списке https://mi-shop.com/ru/shops/

## Mini-Mani

	name=Mini-Mani
	tourism=hostel
	website=http://www.minimani-spb.ru/
	contact:vk=https://vk.com/mini_mani

## Mr.Doors

	name=Mr.Doors
	shop=furniture
	website=https://www.mebel.ru/
	contact:vk=https://vk.com/mr.doors_russia
	opening_hours=10:00-20:00

* `website` можно уточнить на карте и в списке https://www.mebel.ru/shops/sankt-petersburg/

## New Balance

	name=New Balance
	brand=New Balance
	brand:wikidata=Q742988
	shop=shoes
	website=https://newbalance.ru/
	contact:vk=https://vk.com/nb_russia
	opening_hours=10:00-22:00
	operator=ЗАО «Джамилько»
	operator:wikidata=Q4159814

* можно проверить на карте https://newbalance.ru/support/shops/

## OldBoy

	name=OldBoy
	shop=hairdresser
	website=https://oldboybarbershop.com/spb/
	opening_hours=10:00-21:00

* `website` можно уточнить
* у некоторых точек есть `contact:vk`

## Ollis Club

	name=Ollis Club
	amenity=fast_food
	cuisine=pizza
	website=https://www.ollis.ru/
	contact:vk=https://vk.com/ollisclub
	opening_hours=24/7
	operator=ООО «Сытопьяно»

* `website` можно уточнить на карте и в списке https://www.ollis.ru/restaurants/

## OQ

	name=OQ
	shop=copyshop
	website=http://copy.spb.ru/
	contact:vk=https://vk.com/copyspbru

Альтернативные теги:

	name=OQ Express

* `website` можно уточнить на карте и в списке http://copy.spb.ru/about/kontakti/

## Ozon.ru

	name=Ozon Port
	brand=Ozon.ru
	brand:wikidata=Q2365235
	shop=outpost
	website=https://www.ozon.ru/
	contact:vk=https://vk.com/ozon
	opening_hours=Mo-Fr 09:00-21:00; Sa-Su 10:00-20:00
	operator=ООО «Интернет Решения»

* можно проверить в списке и на карте https://www.ozon.ru/context/map/?areaid=5911
* разные названия пунктов выдачи

## Pandora

	name=Pandora
	brand=Pandora
	brand:wikidata=Q2241604
	shop=jewelry
	website=https://www.pandora.net/ru-ru
	opening_hours=11:00-22:00
	operator=АО «ПанКлуб»

* можно проверить поиском на https://www.pandora.net/ru-ru/stores

## Petshop.ru

	name=Petshop.ru
	shop=pet
	website=https://www.petshop.ru/
	contact:vk=https://vk.com/club57
	opening_hours=09:00-22:00

* можно проверить на карте https://www.petshop.ru/about/outpost/

## Pita's

	name=Pita's
	amenity=fast_food
	website=http://www.pitas.ru/
	contact:vk=https://vk.com/pitasstreetfood
	opening_hours=10:00-23:00
	operator=ООО «Питас Стрит»

* можно проверить в списке http://www.pitas.ru/addresses/

## Pizza Hut

	name=Pizza Hut
	brand=Pizza Hut
	brand:wikidata=Q191615
	amenity=fast_food
	cuisine=pizza
	website=http://pizzahut.spb.ru/
	contact:vk=https://vk.com/pizzahutspb

Альтернативные теги:

	opening_hours=Su-Th 10:00-23:00; Fr-Sa 10:00-05:00

## Plaza

	name=Plaza
	shop=kitchen
	website=http://www.plazareal.ru/
	opening_hours=Mo-Fr 10:00-20:00; Sa-Su 11:00-19:00
	operator=ООО «Уна Фаббрика»

* `website` можно уточнить на карте и в списке http://www.plazareal.ru/kontakty/sankt-peterburg/

## Prisma

	name=Prisma
	brand=Prisma
	brand:wikidata=Q12047031
	shop=supermarket
	website=http://www.prismamarket.ru/
	contact:vk=https://vk.com/prisma
	opening_hours=07:00-23:00
	operator=ООО «Призма»

* `website` можно уточнить в списке и на карте http://www.prismamarket.ru/misc/nashi-magaziny

## re:Store

	name=re:Store
	brand=re:Store
	brand:wikidata=Q7486956
	website=https://www.re-store.ru/
	shop=computer
	contact:vk=https://vk.com/re_store
	opening_hours=10:00-22:00

* можно проверить в списке и на карте https://www.re-store.ru/shops/

## Redmond

	name=Redmond
	shop=electronics
	website=https://multivarka.pro/
	contact:vk=https://vk.com/multipro
	opening_hours=10:00-22:00
	operator=ООО «Гравилат»

* `website` можно уточнить на карте и в списке https://multivarka.pro/shops/sankt_peterburg/

## Rieker

	name=Rieker
	brand=Rieker
	brand:wikidata=Q2152193
	shop=shoes

Альтернативные теги:

	opening_hours=10:00-20:00
	opening_hours=10:00-21:00

	operator=ООО «Атон»
	operator=ООО «Эридана»

* на сайте https://www.rieker.com/russisch нет информации о магазинах
* разные операторы

## Rinaldi

	name=Rinaldi
	tourism=hotel
	website=http://rinaldi.ru/
	contact:vk=https://vk.com/id152254413

* `website` можно уточнить в списке и на карте http://rinaldi.ru/hotels

## Royal Thai

	name=Royal Thai
	shop=massage
	massage=thai
	website=http://www.royalthai.ru/
	contact:vk=https://vk.com/royal_thai

* `website` можно уточнить в списке http://www.royalthai.ru/#anchor-screen-8

## S2B

	name=S2B
	shop=tobacco
	website=http://shisha-rf.com/
	contact:vk=https://vk.com/shisharfshop

* можно проверить на карте и в списке http://shisha-rf.com/shops

## Samsonite

	name=Samsonite
	brand=Samsonite
	brand:wikidata=Q1203426
	shop=bag
	website=https://www.samsonite.ru/

* можно проверить на карте и в списке https://www.samsonite.ru/store-locator/

## Samsonov Hotels

	name=Samsonov Hotels
	tourism=hotel
	website=http://sutki78.ru/
	contact:vk=https://vk.com/oteli_peterburg

* `website` можно уточнить на карте http://sutki78.ru/
* возможно также http://www.pitersutki.ru/
* у некоторых `contact:vk` можно уточнить

## SkillSet

	name=SkillSet
	amenity=language_school
	website=http://skillset.ru/
	contact:vk=https://vk.com/skillsetspb

## Sofia

	name=Sofia
	shop=shoes
	website=http://www.sofiashoes.ru/
	contact:vk=https://vk.com/sofia_obuv_spb

Альтернативные теги:

	opening_hours=10:00-21:00
	opening_hours=Mo-Fr 10:00-22:00; Sa-Su 10:00-20:00

* можно проверить на карте и в списке http://www.sofiashoes.ru/shops/

## Solo

	name=Solo
	tourism=hotel
	website=http://solo-hotel.com/
	contact:vk=https://vk.com/club6727081

* `name` и `website` можно уточнить в списке http://solo-hotel.com/

## Sonata

см. *Соната*

## Spar

	name=Spar
	brand=Spar
	brand:wikidata=Q610492
	shop=supermarket
	website=http://www.sparspb.ru/
	contact:vk=https://vk.com/spar_spb
	opening_hours=24/7
	operator=ООО «ТД Интерторг»
	operator:wikidata=Q16655827

Альтернативные теги:

	name=Spar Express

* `website` можно уточнить в списке и на карте http://www.sparspb.ru/index/magaziny

## Street

	name=Street
	amenity=fast_food
	fast_food=cafeteria
	website=http://stolovayastreet.ru/
	opening_hours=09:00-22:00

* можно проверить в списке и на карте http://stolovayastreet.ru/#contact

## Subway

	name=Subway
	brand=Subway
	brand:wikidata=Q244457
	amenity=fast_food
	website=http://subway.ru/
	contact:vk=https://vk.com/subwayrussia

Альтернативные теги:

	name:en=Subway
	name:ru=Сабвэй

* можно проверить на карте http://subway.ru/stores/?q=%D0%A1%D0%B0%D0%BD%D0%BA%D1%82-%D0%9F%D0%B5%D1%82%D0%B5%D1%80%D0%B1%D1%83%D1%80%D0%B3
* разные операторы, у некоторых свои сайты и прочие контактные данные
* в качестве основного названия используют "Subway", у некоторых дополнительно используется "Сабвэй", у других - нет

## SuperHostel

см. *СуперХостел*

## Surf Coffee

	name=Surf Coffee
	amenity=cafe
	website=http://surfcoffee.ru/
	contact:vk=https://vk.com/surfcoffeespb

* `name` и `website` можно уточнить для некоторых точек

## SПБ

	name=SПБ
	amenity=bar
	website=http://www.barspb.ru/
	contact:vk=https://vk.com/barspb_official
	opening_hours=Su-Th 12:00-02:00, Fr-Sa 12:00-05:00

* можно проверить на карте http://www.barspb.ru/contacts.html

## Tele2

	name=Tele2
	brand=Tele2
	brand:wikidata=Q309865
	shop=mobile_phone
	office=telecommunication
	website=https://spb.tele2.ru/
	contact:vk=https://vk.com/tele2

Альтернативные теги:

	opening_hours=09:00-21:00
	opening_hours=09:00-22:00
	opening_hours=10:00-21:00

* можно проверить на карте https://spb.tele2.ru/offices
* `contact:vk=https://vk.com/tele2spb` больше не поддерживается

## Thomas Münz

	name=Thomas Münz
	shop=shoes
	website=http://thomas-muenz.ru/
	contact:vk=https://vk.com/thomas_munz
	opening_hours=10:00-22:00

* `website` можно уточнить в списке и на карте http://thomas-muenz.ru/salony

## Timberland

	name=Timberland
	brand=Timberland
	brand:wikidata=Q1539185
	shop=shoes
	website=https://timberland.ru/
	contact:vk=https://vk.com/timbrussia
	opening_hours=10:00-22:00
	operator=ЗАО «Джамилько»
	operator:wikidata=Q4159814

* можно проверить на карте https://timberland.ru/helpdesc/shops/sankt_peterburg/

## Time for Coffee

	name=Time for Coffee
	amenity=fast_food
	cuisine=coffee_shop
	contact:vk=https://vk.com/time_forcoffee
	opening_hours=08:00-22:00

## TJ Collection

	name=TJ Collection
	shop=shoes
	website=http://tjcollection.ru/
	contact:vk=https://vk.com/public30303738
	opening_hours=10:00-22:00
	operator=ООО «КЛ Групп»

* можно проверить на карте и в списке https://tjcollection.ru/shops/russia/sankt-peterburg/
* есть отдельно сайт онлайн-магазина https://www.tjonline.ru/

## Togas

	name=Togas
	shop=bed
	website=https://www.togas.com/ru/
	contact:vk=https://vk.com/togastextile
	opening_hours=10:00-22:00
	operator=ИП «Тогкас Павлос»

* можно проверить на карте https://www.togas.com/ru/boutiques/

## TopEr

	name=TopEr
	amenity=restaurant
	cuisine=chinese
	website=https://toperspb.ru/
	contact:vk=https://vk.com/toperchina

* `website` можно уточнить в списке и на карте https://toperspb.ru/kontakty/

## Trives

	name=Trives
	shop=medical_supply
	medical_supply=orthopedic
	website=http://trives-shop.ru/
	contact:vk=https://vk.com/club_trives_spb
	opening_hours=09:00-21:00

* `website` можно уточнить в списке и на карте http://trives-shop.ru/store/

## Truvor

	name=Truvor
	shop=clothes
	clothes=men
	website=https://www.truvor.ru/
	contact:vk=https://vk.com/truvor_truvor
	opening_hours=10:00-21:00

Альтернативные теги:

	website=http://truvor-sale.ru/

* можно проверить в списке и на карте https://www.truvor.ru/where-buy/
* https://www.truvor.ru/ - сайт фабрики
* http://truvor-sale.ru/ - сайт одной из сетей сродаж

## TUI

	name=TUI
	brand=TUI
	brand:wikidata=Q573103
	shop=travel_agency
	website=https://www.tui.ru/
	contact:vk=https://vk.com/tuiru

* можно проверить на карте https://www.tui.ru/where-to-buy/?t=map

## Unique Fabric

	name=Unique Fabric
	shop=clothes
	website=https://uniquefabric.ru/
	contact:vk=https://vk.com/uniquefabric
	opening_hours=10:00-22:00

* можно проверить в списке https://uniquefabric.ru/help/stores/

## Vardex

	name=Vardex
	shop=e-cigarette
	website=https://www.vardex.ru/
	contact:vk=https://vk.com/vardex_ru
	opening_hours=Mo-Fr 09:00-21:00; Sa-Su 10:00-21:00

* `website` можно уточнить на карте https://www.vardex.ru/shops/sankt-petersburg/

## Velardi

	name=Velardi
	shop=hairdresser
	website=http://velardi.ru/
	contact:vk=https://vk.com/velardi1
	opening_hours=09:00-21:00

* `website` можно уточнить в списке http://velardi.ru/content/blogsection/6/44/

## Vinissimo

	name=Vinissimo
	shop=alcohol
	website=http://www.vinissimo.biz/
	contact:vk=https://vk.com/vinissimoshop
	opening_hours=11:00-22:00

* `website` можно уточнить в списке http://www.vinissimo.biz/st-petersburg/wine/

## Vintage

	name=Vintage
	shop=tea
	website=http://tea-coffee.info/
	contact:vk=https://vk.com/tm_vintage

* можно проверить в списке http://tea-coffee.info/shops/sankt-peterburg-i-lo/

## Watsons

	name=Watsons
	brand=Watsons
	brand:wikidata=Q7974785
	shop=chemist
	website=https://watsons.com.ru/
	contact:vk=https://vk.com/watsonsrussia
	opening_hours=09:00-22:00
	operator=ООО «Спектр Групп»

Альтернативные теги:

	name=Спектр

* `website` можно уточнить на карте https://watsons.com.ru/adresa-magazinov
* *Спектр* [был куплен *Watsons*](https://watsons.com.ru/o-kompanii), возможно ещё остались старые названия

## Wildberries

	name=Wildberries
	brand=Wildberries
	brand:wikidata=Q24933714
	shop=clothes
	reservation=required
	website=https://www.wildberries.ru/
	contact:vk=https://vk.com/wildberries_shop

* можно проверить в списке https://www.wildberries.ru/services/punkty-samovyvoza/sankt-peterburg

## Zeiss

	name=Zeiss
	brand=Zeiss
	brand:wikidata=Q282186
	shop=optician
	website=http://newglance.ru/
	operator=ООО «Зайди — увидишь»

Альтернативные теги:

	opening_hours=Mo-Sa 10:00-21:00; Su 11:00-20:00
	opening_hours=Mo-Fr 10:00-21:00; Sa 10:00-20:00; Su 11:00-19:00

* `website` можно уточнить в списке http://newglance.ru/nashi-salony/

## А-Мега

	name=А-Мега
	amenity=pharmacy
	website=https://www.aptekamega.ru/
	contact:vk=https://vk.com/apteka_mega

* можно проверить на карте https://www.aptekamega.ru/pharmacies/

## Абсолют Банк

	name=Абсолют Банк
	brand=Абсолют Банк
	brand:wikidata=Q4055122
	amenity=bank
	website=https://absolutbank.ru/
	contact:vk=https://vk.com/absolutbankru
	opening_hours=Mo-Fr 09:00-20:00; Sa 10:00-16:00
	operator=ФАКБ «Абсолют Банк» (ПАО)
	operator:wikidata=Q4055122

* можно проверить на карте https://absolutbank.ru/about/offices-atms/

## АВ Daily

см. *Азбука вкуса*

## Адамас

	name=Адамас
	shop=jewelry
	website=https://www.adamas.ru/
	contact:vk=https://vk.com/adamas.club
	opening_hours=10:00-21:00
	operator=ООО «Адамас-Ювелирторг»

* `website` можно уточнить на карте и в списке https://www.adamas.ru/buy/sankt-peterburg/

## Адвекс

	name=Адвекс
	office=estate_agent
	website=http://www.advecs.com/
	contact:vk=https://vk.com/advecscorp

* `website` можно уточнить в списке и на карте http://www.advecs.com/offices/

## Азбука вкуса

	name=Азбука вкуса
	brand=Азбука вкуса
	brand:wikidata=Q4058209
	shop=supermarket
	website=https://spb.av.ru/
	contact:vk=https://vk.com/azbukavkusaspb
	operator=ООО «Городской супермаркет»

Альтернативные теги:

	name=АВ Daily
	shop=convenience

* `website` можно уточнить на карте и в списке https://av.ru/shops/#spb

## Алекс Отель

	name=Алекс Отель
	tourism=hotel
	website=https://alexhotels.ru/
	contact:vk=https://vk.com/alexhotelspb

* `website` можно уточнить в списке https://alexhotels.ru/hotels/

## Александр Недвижимость

	name=Александр Недвижимость
	office=estate_agent
	website=https://anspb.ru/

* `website` можно уточнить в списке и на карте https://anspb.ru/contacts.php

## Алоэ

	name=Алоэ
	amenity=pharmacy
	website=https://aloeapteka.ru/
	contact:vk=https://vk.com/aptekaaloea
	opening_hours=Mo-Fr 09:00-22:00; Sa-Su 09:00-21:00

* можно проверить в списке и на карте https://aloeapteka.ru/adresa/sankt-peterburg/

## Альфа-Банк

	name=Альфа-Банк
	brand=Альфа-Банк
	brand:wikidata=Q1377835
	amenity=bank
	website=https://alfabank.ru/
	contact:vk=https://vk.com/alfabank
	opening_hours=Mo-Fr 09:00-20:00; Sa 10:00-19:00
	operator=АО «Альфа-Банк»
	operator:wikidata=Q1377835

* `website` можно уточнить в списке https://alfabank.ru/office/peterburg/list/
* можно проверить на карте https://alfabank.ru/office/peterburg/map/
* часто бывает два разных отделения в одном месте

## Ароматный мир

	name=Ароматный мир
	shop=alcohol
	website=https://amwine.ru/
	contact:vk=https://vk.com/amwineru
	opening_hours=10:00-22:00

Альтернативные теги:

	operator=ООО «АМ-2 СПб»
	operator=ООО «АМ-4 СПб»
	operator=ООО «Винторг»
	operator=ООО «ВТ-Центр»
	operator=ООО «Объект Плюс»

* `website` можно уточнить поиском https://amwine.ru/customers/shops/
* разные операторы, у каждого может быть по несколько магазинов

## Архитектор красоты

	name=Архитектор красоты
	shop=hairdresser_supply
	website=http://arhitektorkrasoty.ru/
	contact:vk=https://vk.com/arhitektorkrasoty

* `website` можно уточнить в списке и на карте http://arhitektorkrasoty.ru/kontakty/?city=2&metro=0

## Атмосфера

	name=Атмосфера
	tourism=hotel
	contact:vk=https://vk.com/ahotel
	website=http://a-hotel.spb.ru/

* `website` можно уточнить в списке http://a-hotel.spb.ru/#list
* `website` можно уточнить на карте http://a-hotel.spb.ru/#map

## Белорусский дворик

	name=Белорусский дворик
	shop=convenience
	website=http://belorusdvorik.ru/
	contact:vk=https://vk.com/belorusdvorik

* можно проверить в списке https://vk.com/topic-165282695_38691225
* здесь старые адреса http://belorusdvorik.ru/adresa-vsekh-magazinov.html

## Билайн

	name=Билайн
	brand=Билайн
	brand:wikidata=Q402880
	shop=mobile_phone
	office=telecommunication
	website=https://spb.beeline.ru/
	contact:vk=https://vk.com/beeline

Альтернативные теги:

	opening_hours=09:00-21:00
	opening_hours=10:00-22:00

* можно проверить на карте https://spb.beeline.ru/customers/beeline-on-map/

## Благо

	name=Благо
	shop=pawnbroker
	website=https://www.lombardblago.ru/
	contact:vk=https://vk.com/lombardblago
	opening_hours=09:00-20:00
	operator=ООО ломбард «Благо Кредит»

* можно проверить на карте https://www.lombardblago.ru/adresa-lombardov?region_id=126

## Большой праздник

	name=Большой праздник
	alt_name=Планета фейерверков
	shop=pyrotechnics
	website=http://www.bolshoy.ru/
	website2=http://www.planetaf.ru/
	contact:vk=https://vk.com/bolshoyru
	opening_hours=10:00-21:00

* можно проверить на карте и в списке http://www.bolshoy.ru/contacts/sankt-peterburg/

## Британские пекарни

	name=Британские пекарни
	amenity=cafe
	website=https://british-bakery.ru/
	contact:vk=https://vk.com/club_british_bakery
	opening_hours=08:30-22:00

* `website` можно уточнить на карте https://british-bakery.ru/shops/

## Брынза

	name=Брынза
	amenity=cafe
	website=https://cafebrynza.ru/
	opening_hours=10:00-23:00
	operator=ООО «Союз»

* можно проверить в списке и на карте https://cafebrynza.ru/restorany

## Буду мамой

	name=Буду мамой
	shop=clothes
	clothes=maternity
	website=http://www.budumamoy.ru/
	contact:vk=https://vk.com/budumamoy
	opening_hours=10:00-21:00

* можно проверить на карте и в списке http://www.budumamoy.ru/pages/roznichnie-magazini_114/item_1/

## Буквоед

	name=Буквоед
	brand=Буквоед
	brand:wikidata=Q4098549
	shop=books
	website=https://www.bookvoed.ru/
	contact:vk=https://vk.com/bookvoed

* `website` можно уточнить на карте https://www.bookvoed.ru/shops
* `website` можно уточнить в списке https://www.bookvoed.ru/shops#addresses

## Булки-баранки

	name=Булки-баранки
	shop=bakery
	website=http://bulki-baranki.com/
	contact:vk=https://vk.com/bulkib
	opening_hours=08:00-22:00
	operator=ООО «Арсенал Инвест»

* можно проверить в списке http://bulki-baranki.com/addresses/

## Булочные Ф. Вольчека

	name=Булочные Ф. Вольчека
	shop=bakery
	website=http://fvolchek.ru/
	contact:vk=https://vk.com/f.volchek
	opening_hours=08:00-22:00

Альтернативные теги:

	name=Пекарня Ф. Вольчека

* можно проверить в списке http://fvolchek.ru/
* возможно различное написание названия

## Бургер Кинг

	name=Бургер Кинг
	int_name=Burger King
	brand=Burger King
	brand:wikidata=Q177054
	amenity=fast_food
	website=https://burgerking.ru/
	contact:vk=https://vk.com/burgerking
	operator=ООО «Бургер Рус»

* можно проверить на карте https://burgerking.ru/restaurants

## Буше

	name=Буше
	amenity=cafe
	shop=pastry
	website=http://bushe.ru/
	contact:vk=https://vk.com/bushebakery
	operator=ООО «ПКД «Венец»

Альтернативные теги:

	opening_hours=08:00-22:00
	opening_hours=08:00-23:00

* можно проверить на карте http://bushe.ru/places

## Вака

	name=Вака
	shop=pet
	website=http://www.vaka.ru/
	contact:vk=https://vk.com/zoomagazinvaka
	operator=ООО «Биосфера»

Альтернативные теги:

	opening_hours=10:00-21:00
	opening_hours=11:00-20:00
	opening_hours=11:00-21:00

* `website` для некоторых точек можно уточнить на карте и в списке http://www.vaka.ru/vakashops

## Великолукский мясокомбинат

	name=Великолукский мясокомбинат
	brand=Великолукский мясокомбинат
	brand:wikidata=Q18401767
	shop=butcher
	opening_hours=Mo 10:00-22:00; Tu-Sa 09:00-22:00; Su 09:00-21:00

Альтернативные теги:

	operator=ООО «Меркурий»

* http://www.vlmk.spb.ru/ не подходит, так как относится к самому комбинату
* https://vk.com/vlmkspb не подходит, так как относится к самому комбинату... хотя кое-что про магазины там говорится

## Венская кофейня

	name=Венская кофейня
	brand=Coffeeshop Company
	brand:wikidata=Q873767
	amenity=cafe
	cuisine=coffee_shop
	website=http://www.coffeeshopcompany.ru/
	contact:vk=https://vk.com/club10239197

* `website` можно уточнить на карте http://www.coffeeshopcompany.ru/address/

## Верный

	name=Верный
	shop=supermarket
	website=http://www.verno-info.ru/
	contact:vk=https://vk.com/vernoinfo
	opening_hours=09:00-22:00
	operator=ООО «Союз Св. Иоанна Воина»

* `website` можно уточнить на карте http://www.verno-info.ru/shops/

## Вижен сервис

см. *Невская оптика*

## Викинг

	name=Викинг
	brand=Викинг
	brand:wikidata=Q37650790
	amenity=bureau_de_change
	website=http://www.vikingbank.ru/

Альтернативные теги:

	amenity=bank

* можно проверить в списке http://www.vikingbank.ru/viking_new2.nsf/ru/offices

## Виллерой и Бох

	name=Виллерой и Бох
	int_name=Villeroy & Boch
	brand=Villeroy & Boch
	brand:wikidata=Q705042
	shop=houseware
	website=https://www.villeroy-boch.ru/

* можно проверить на карте https://www.villeroy-boch.ru/adresa/stolovaja-posuda.html

## Вита Экспресс

	name=Вита Экспресс
	amenity=pharmacy
	website=https://vitaexpress.ru/
	contact:vk=https://vk.com/aptekivita
	opening_hours=08:00-22:00

* можно проверить на карте https://vitaexpress.ru/search-apt/

## Водопад

	name=Водопад
	shop=bathroom_furnishing
	website=https://vodopad.ru/
	contact:vk=https://vk.com/vodopadspb

* можно проверить на карте и в списке https://vodopad.ru/info/shops.php

## Восточный

	name=Восточный
	brand=Восточный банк
	brand:wikidata=Q4126657
	amenity=bank
	website=https://www.vostbank.ru/
	contact:vk=https://vk.com/vostbankru
	operator=ПАО КБ «Восточный»
	operator:wikidata=Q4126657

Альтернативные теги:

	opening_hours=Mo-Fr 10:00-19:00
	opening_hours=Mo-Fr 09:00-20:00; Sa 10:00-20:00; Su 10:00-16:00

* можно проверить на карте https://www.vostbank.ru/office/

## ВотОнЯ

	name=ВотОнЯ
	shop=baby_goods
	website=https://www.votonia.ru/
	contact:vk=https://vk.com/votonia
	opening_hours=09:00-21:00

* `website` можно уточнить в списке https://www.votonia.ru/adresa-magazinov/

## Всё что нужно

	name=Всё что нужно
	shop=variety_store
	opening_hours=09:00-22:00

* вместо накоторых открылись *Закка*

## ВТБ

	name=ВТБ
	brand=ВТБ
	brand:wikidata=Q1549389
	amenity=bank
	website=https://www.vtb.ru/
	contact:vk=https://vk.com/vtb
	opening_hours=Mo-Fr 10:00-19:00
	operator=Банк ВТБ (публичное акционерное общество)
	operator:wikidata=Q1549389

* `website` можно уточнить на карте и в списке https://www.vtb.ru/o-banke/kontakty/otdeleniya/
* `website` для Private Banking можно уточнить на карте https://www.vtb.ru/private-banking/ofisy-private-banking/

## Газпромбанк

	name=Газпромбанк
	brand=Газпромбанк
	brand:wikidata=Q1924338
	amenity=bank
	website=http://www.gazprombank.ru/
	operator=«Газпромбанк» (Акционерное общество)
	operator:wikidata=Q1924338

* можно проверить на карте http://www.gazprombank.ru/additional_office/

## Гарсон

	name=Гарсон
	amenity=cafe
	shop=bakery
	website=https://www.garcon.ru/
	contact:vk=https://vk.com/club52932043
	opening_hours=09:00-21:00

Альтернативные теги:

	shop=pastry
	int_name=Garçon

## География

	name=География
	shop=travel_agency
	website=https://www.geograftour.com/
	contact:vk=https://vk.com/geografspb
	opening_hours=Mo-Fr 10:00-19:00

* `website` можно уточнить на карте и в списке https://www.geograftour.com/office/Sankt-Peterburg/

## Глобэкс

	name=Глобэкс
	brand=Глобэкс
	brand:wikidata=Q4139946
	amenity=bank
	website=https://globexbank.ru/
	opening_hours=Mo-Fr 09:00-19:00; Sa 09:00-16:30

* `website` можно уточнить на карте https://globexbank.ru/about/offices/

## Горздрав

	name=Горздрав
	amenity=pharmacy
	website=https://gorzdrav.org/
	contact:vk=https://vk.com/gorzdrav

Альтернативные теги:

	operator=АО «Фармакор»
	operator=ООО «Аптека 36,6»
	operator:wikidata=Q226889

	opening_hours=Mo-Fr 08:00-22:00; Sa-Su,PH 09:00-21:00
	opening_hours=Mo-Fr 09:00-22:00; Sa-Su,PH 09:00-21:00

* `website` можно уточнить на карте https://gorzdrav.org/apteki/map/
* разные операторы
* [36,6 купили аптеки у Фармакора](https://www.dp.ru/a/2017/09/08/Aptechnuju_set_Farmakor)

## Городские кондитерские №1

	name=Городские кондитерские №1
	amenity=cafe
	website=http://www.konditer1.ru/
	contact:vk=https://vk.com/citycakes
	opening_hours=08:30-22:00
	operator=ООО «Нептун»

* можно проверить на карте и в списке http://www.konditer1.ru/adresses.html

## Горячие туры

	name=Горячие туры
	shop=travel_agency
	website=http://www.hottspb.ru/
	contact:vk=https://vk.com/hottmanagementcompany
	brand=Горячие туры

* `name` и `website` можно уточнить в списке и на карте http://www.hottspb.ru/ofisi_na_glavnoy/

## Градусы всего мира

	name=Градусы всего мира
	shop=alcohol
	website=http://www.gradusi.net/
	contact:vk=https://vk.com/gradusi_market
	opening_hours=09:00-22:00

Альтернативные теги:

	operator=ООО «Брют»
	operator=ООО «Винтаж»

* `website` можно уточнить на карте и в списке http://www.gradusi.net/shops/

## Две палочки

	name=Две палочки
	alt_name=2 палочки
	amenity=restaurant
	website=http://dvepalochki.ru/
	contact:vk=https://vk.com/dvepalochki

* `website` можно уточнить в списке и на карте http://dvepalochki.ru/restaurants/

## Деньга

	name=Деньга
	shop=money_lender
	website=https://denga.ru/spb/
	contact:vk=https://vk.com/dengazaim

## ДентИдеал

	name=ДентИдеал
	amenity=dentist
	website=http://www.dentideal.ru/
	contact:vk=https://vk.com/dentideal
	opening_hours=Mo-Fr 09:00-20:00; Sa 09:00-15:00
	operator=ООО «ДентИдеал Центр»

* `website` можно уточнить на карте и в списке http://www.dentideal.ru/clinics/

## Дикая Орхидея

	name=Дикая Орхидея
	shop=clothes
	website=https://www.mywildorchid.ru/
	opening_hours=Mo-Su 10:00-22:00
	operator=ООО «РозТех»

* можно проверить на карте https://www.mywildorchid.ru/findstore/

## Дикси

	name=Дикси
	brand=Дикси
	brand:wikidata=Q4161561
	shop=supermarket
	website=https://dixy.ru/
	contact:vk=https://vk.com/dixyclub
	operator=АО «Дикси Юг»

Альтернативные теги:

	opening_hours=08:00-23:00
	opening_hours=24/7

* можно проверить на карте https://dixy.ru/nearest-shop/
* на карте у некоторых кривые координаты, может понадобиться поиск

## До зарплаты

	name=До зарплаты
	shop=money_lender
	website=https://dozarplati.com/
	contact:vk=https://vk.com/dozarplaticom

* можно проверить на карте https://dozarplati.com/about/branches

## Доверие

	name=Доверие
	shop=pawnbroker
	website=https://doveriegroup.ru/
	contact:vk=https://vk.com/lombard_doverie
	opening_hours=08:00-20:00

* `website` можно уточнить в списке и на карте https://doveriegroup.ru/contacts

## Додо Пицца

	name=Додо Пицца
	amenity=fast_food
	cuisine=pizza
	website=https://dodopizza.ru/
	contact:vk=https://vk.com/dodospb

Альтернативные теги:

	opening_hours=09:00-01:00
	opening_hours=10:00-23:00

* `website` можно уточнить в списке https://dodopizza.ru/peterburg/contacts

## Доктор Столетов

	name=Доктор Столетов
	amenity=pharmacy
	website=https://stoletov.ru/
	contact:vk=https://vk.com/drstoletov

Альтернативные теги:

	opening_hours=Mo-Fr 07:00-23:00; Sa-Su 08:00-22:00
	opening_hours=24/7
	operator=ООО «Эркафарм Санкт-Петербург»
	operator=ООО «Эркафарм Северо-Запад»

* можно проверить на карте https://stoletov.ru/map/

## Дом книги

	name=Дом книги
	shop=books
	website=http://www.dk-spb.ru/
	opening_hours=11:00-20:00
	operator=ОАО ТФ «СПб Дом книги»

* можно проверить на карте http://www.dk-spb.ru/homebook.php?part=contact&idsub=4
* это НЕ *Дом книги* в *здании Зингера*

## Друзья

	name=Друзья
	tourism=hotel
	website=http://www.friendsplace.ru/
	contact:vk=https://vk.com/spbfriends

Альтернативные теги:

	tourism=hostel

* `name` и `website` можно уточнить на карте и в списке http://www.friendsplace.ru/spb/

## Дэфо

	name=Дэфо
	shop=furniture
	website=http://spb.defo.ru/
	contact:vk=https://vk.com/defo_mebel
	opening_hours=Mo-Fr 09:00-21:00; Sa-Su 10:00-19:00

* `website` можно уточнить на карте и в списке http://spb.defo.ru/contacts/

## Евразия

	name=Евразия
	amenity=restaurant
	website=https://evrasia.spb.ru/
	contact:vk=https://vk.com/evrasiaspb
	opening_hours=12:00-05:00

* можно проверить на карте и в списке https://evrasia.spb.ru/restaurants/
* разные операторы

## ЕвропаПроф

	name=ЕвропаПроф
	shop=hairdresser_supply
	website=https://europaprof.ru/
	opening_hours=10:00-21:00
	operator=ИП «Зильвестр А.Р.»

* можно проверить на карте и в списке https://europaprof.ru/magaziny

## Евросеть

	name=Евросеть
	brand=Евросеть
	brand:wikidata=Q65310
	shop=mobile_phone
	website=https://euroset.ru/
	contact:vk=https://vk.com/euroset_club
	opening_hours=09:45-21:15

* `website` можно уточнить на карте и в списке https://euroset.ru/sankt-peterburg/shops/

## Жан Руа

	name=Жан Руа
	shop=bakery
	website=http://zhanrua.ru/
	contact:vk=https://vk.com/zhanrua
	opening_hours=09:00-22:00

## Зайди — увидишь

	name=Зайди — увидишь
	shop=optician
	website=http://newglance.ru/
	opening_hours=Mo-Sa 10:00-21:00; Su 11:00-20:00
	operator=ООО «Зайди — увидишь»

* `website` можно уточнить в списке http://newglance.ru/nashi-salony/

## Заначка

	name=Заначка
	shop=money_lender
	website=https://zdengi.ru/
	contact:vk=https://vk.com/zanachka_spb

* можно проверить на карте https://zdengi.ru/kontakty/

## Заубер Банк

	amenity=bank
	name=Заубер Банк
	website=https://www.sauberbank.com/
	opening_hours=09:30-20:30

Альтернативные теги:

	amenity=bureau_de_change

* можно проверить в списке https://www.sauberbank.com/spb/contacts/petersburg/

## Зерна

	name=Зерна
	amenity=fast_food
	cuisine=coffee_shop
	contact:vk=https://vk.com/zernacafe

* можно проверить в списке https://vk.com/zernacafe

## Золотая рыбка

	name=Золотая рыбка
	shop=pet
	website=https://goldfish.ru/
	contact:vk=https://vk.com/goldfish_club
	opening_hours=10:00-21:00

* можно проверить на карте https://goldfish.ru/retail/shops/

## Золотой

	name=Золотой
	shop=jewelry
	website=https://www.gold585.ru/
	opening_hours=10:00-22:00

* можно проверить на карте и в списке https://www.gold585.ru/contacts/shops/sankt-peterburg/

## Императорский фарфор

	name=Императорский фарфор
	brand=Императорский фарфоровый завод
	brand:wikidata=Q1303887
	shop=houseware
	website=http://www.ipm.ru/
	contact:vk=https://vk.com/ipm_ru

* `website` можно уточнить на карте и в списке http://www.ipm.ru/shops/

## Империя платья

	name=Империя платья
	shop=clothes
	website=http://империяплатья.рф/
	contact:vk=https://vk.com/imperiaplatya

* можно проверить в списке http://империяплатья.рф/page/adresa-magazinov

## Инвитро

	name=Инвитро
	brand=Инвитро
	brand:wikidata=Q4200546
	amenity=doctors
	website=https://www.invitro.ru/
	contact:vk=https://vk.com/invitro
	operator=ООО «Инвитро СПб»

* `website` можно уточнить на карте https://www.invitro.ru/offices/piter/

## Индустрия красоты

	name=Индустрия красоты
	name:ru=Индустрия красоты
	name:en=Industry of Beauty
	shop=cosmetics
	website=http://ib-spb.ru/
	contact:vk=https://vk.com/industry_of_beauty
	opening_hours=10:00-21:00
	operator=ООО «Индустрия красоты»

* можно проверить на карте и в списке http://ib-spb.ru/shops

## Интан

	name=Интан
	amenity=dentist
	website=https://intan.ru/
	contact:vk=https://vk.com/intan
	opening_hours=Mo-Sa 09:00-22:00; Su 10:00-20:00

* `website` можно уточнить в списке https://intan.ru/

## Искусница

	name=Искусница
	shop=sewing
	website=https://iskusnica.spb.ru/
	contact:vk=https://vk.com/iskusnicaspbru

* `website` можно уточнить в списке и на карте https://iskusnica.spb.ru/shops/

## Калевала

	name=Калевала
	shop=shoes
	website=http://www.kalevala-spb.ru/
	contact:vk=https://vk.com/club45625838
	opening_hours=Mo-Sa 10:00-21:00; Su 11:00-20:00
	operator=ООО «Калевала»

* можно проверить на карте и в списке http://www.kalevala-spb.ru/shoes-shops/

## Калейдоскоп напитков мира

	name=Калейдоскоп напитков мира
	shop=alcohol
	website=http://www.napitkimira.com/
	contact:vk=https://vk.com/napitkimiracom
	opening_hours=10:00-22:00
	operator=ООО «Калейдоскоп»

* можно проверить на карте http://www.napitkimira.com/map/
* `website` можно уточнить в списке http://www.napitkimira.com/magaziny/

## Канцелярская Мекка

	name=Канцелярская Мекка
	shop=stationery
	website=https://www.mekka-spb.ru/
	contact:vk=https://vk.com/mekkakanc
	opening_hours=09:00-21:00

* можно проверить на карте и в списке https://www.mekka-spb.ru/about/

## Кей

	name=Кей
	brand=Кей
	brand:wikidata=Q4218938
	shop=computer
	website=https://key.ru/
	contact:vk=https://vk.com/key.russia
	operator=ООО «Кей»

* `website` можно уточнить на карте https://key.ru/magaziny/

## Кетовский коммерческий банк

	name=Кетовский коммерческий банк
	amenity=bank
	website=http://www.bank-45.ru/
	contact:vk=https://vk.com/bank45

* можно проверить в списке http://www.bank-45.ru/index.php/info/2014-02-14-08-56-51.html

## Ки-До

	name=Ки-До
	amenity=restaurant
	website=https://kido.ru/
	contact:vk=https://vk.com/ki_do

* `website` можно уточнить на карте и в списке https://kido.ru/places.html

## Кладовая здоровья

	name=Кладовая здоровья
	shop=medical_supply
	medical_supply=orthopedic
	website=https://kladzdor.ru/
	contact:vk=https://vk.com/kladovaya_zdorovya
	opening_hours=Mo-Fr 10:00-21:00; Sa 11:00-20:00; Su 11:00-19:00

* `website` можно уточнить на карте и в списке https://kladzdor.ru/adresa-magazinov/

## Компас здоровья

	name=Компас здоровья
	shop=health_food
	website=http://www.smkfarm.ru/
	contact:vk=https://vk.com/eco_bio_market
	opening_hours=09:00-21:00
	operator=ИП Круглова Екатерина Сергеевна

* можно проверить на карте и в списке http://www.smkfarm.ru/contacts/stores/rossiya/sank-peterburg/
* сайт http://kz-spb.ru/ не работает, но упоминается на vk

## Комус

	name=Комус
	brand=Комус
	brand:wikidata=Q4230314
	shop=stationery
	website=https://www.komus.ru/
	contact:vk=https://vk.com/komus_club
	opening_hours=Mo-Fr 09:00-21:00; Sa-Su 10:00-20:00
	operator=ООО «Комус-Петербург»

* `website` можно уточнить на карте и в списке https://www.komus.ru/store/

## Комфорт Все сезоны

см. *All Seasons Комфорт*

## Контакт бар

	name=Контакт бар
	amenity=bar
	website=http://kontaktbar.ru/
	contact:vk=https://vk.com/kontaktbar
	operator=ООО «Альфа Бар»

Альтернативные теги:

	opening_hours=Su-Th 12:00-02:00, Fr-Sa 12:00-06:00
	opening_hours=Mo-Fr 13:00-06:00, Sa-Su 14:00-06:00

* `website` можно уточнить в списке http://kontaktbar.ru/bars/

## Копирка

	name=Копирка
	shop=copyshop
	website=https://kopirka.ru/

* `website` можно уточнить на карте и в списке https://kopirka.ru/offices-spb/

## Котофей

	name=Котофей
	shop=shoes
	website=http://kotofey.ru/
	contact:vk=https://vk.com/tm_kotofey
	opening_hours=10:00-20:00

Альтернативные теги:

	website=https://www.kotofey-spb.ru/
	contact:vk=https://vk.com/kotofeypiter

* http://kotofey.ru/ и https://vk.com/tm_kotofey - сайты производителя/франшизы
* `website` можно уточнить на карте и в списке http://kotofey.ru/shops/location/%D0%A0%D0%BE%D1%81%D1%81%D0%B8%D1%8F/%D0%A1%D0%B0%D0%BD%D0%BA%D1%82-%D0%BF%D0%B5%D1%82%D0%B5%D1%80%D0%B1%D1%83%D1%80%D0%B3
* https://www.kotofey-spb.ru/ и https://vk.com/kotofeypiter - сайты некоторых (не всех) магазинов СПб
* `website` можно уточнить на карте и в списке https://www.kotofey-spb.ru/contacts/stores/

## Кофе Хауз

	name=Кофе Хауз
	brand=Кофе Хауз
	brand:wikidata=Q4236456
	amenity=cafe
	cuisine=coffee_shop
	website=http://www.coffeehouse.ru/
	contact:vk=https://vk.com/ru_coffeehouse
	operator=ООО «Кофе Хаус. Эспрессо и Капучино Бар»

* можно проверить на карте http://www.coffeehouse.ru/adress/?id_townv=26
* можно проверить в списке http://www.coffeehouse.ru/adress/list/?id_town=26

## Кофеварим

	name=Кофеварим
	amenity=cafe
	contact:vk=https://vk.com/clubkofevarim
	opening_hours=07:00-23:00

* можно проверить в списке https://vk.com/clubkofevarim
* для некоторых точек может быть более правильно `amenity=fast_food`

## Кофейня №1

	name=#Кофейня №1
	alt_name=Кофейня №1
	amenity=cafe
	contact:vk=https://vk.com/coffeen1
	opening_hours=24/7
	website=http://delmar-spb.ru/

* `website` можно уточнить в списке http://delmar-spb.ru/contactsdelmar1/

## Краsивая

	name=Краsивая
	alt_name=Красивая
	shop=clothes
	clothes=women
	website=http://krasarf.ru/
	opening_hours=Mo-Sa 10:00-21:00; Su 11:00-20:00

* можно проверить в списке http://krasarf.ru/feedback

## Красное & Белое

	name=Красное & Белое
	shop=alcohol
	website=https://krasnoeibeloe.ru/
	contact:vk=https://vk.com/krasnoebeloe
	opening_hours=09:00-22:05
	operator=ООО «Альфа-М»

* можно проверить в списке и на карте https://krasnoeibeloe.ru/address/

## Кредит Европа Банк

	name=Кредит Европа Банк
	brand=Credit Europe Bank
	brand:wikidata=Q931726
	amenity=bank
	website=https://www.crediteurope.ru/
	contact:vk=https://vk.com/crediteurope

* можно проверить в списке https://www.crediteurope.ru/branches/list/spb/
* можно проверить на карте https://www.crediteurope.ru/branches/maps/spb/

## Крилони

	name=Крилони
	shop=clothes
	clothes=underwear
	website=http://krilony.ru/
	contact:vk=https://vk.com/club_krilony
	opening_hours=11:00-20:00

* можно проверить в списке и на карте http://www.krilony.ru/index.php?fc=module&module=yandexmap&controller=display

## Кэшпойнт

	name=Кэшпойнт
	shop=money_lender
	website=https://cashpoint-kredit.ru/
	opening_hours=09:00-20:00
	operator=ООО МКК «Траст Альянс»

* можно проверить на карте https://cashpoint-kredit.ru/adresa-microzaimov

## Лабиринт

	name=Лабиринт
	shop=books
	reservation=required
	website=https://www.labirint.ru/
	contact:vk=https://vk.com/labirint_ru
	opening_hours=09:00-21:00
	operator=ООО «Лабиринт.ру»

* можно проверить на карте https://www.labirint.ru/maps/
* `website` можно уточнить, но у них в списке не все пункты

## ЛабСтори

	name=ЛабСтори
	amenity=doctors
	health_facility:type=laboratory
	website=https://www.labstori.ru/
	contact:vk=https://vk.com/labstori
	opening_hours=Mo-Fr 08:00-17:00; Sa 09:00-15:00

* можно проверить на карте https://www.labstori.ru/pacientam/contacts/

## Лавка Пекаря

	name=Лавка Пекаря
	shop=bakery
	website=http://www.lavkapekarya.ru/
	contact:vk=https://vk.com/lavka_pekarya
	opening_hours=08:00-21:00
	operator=АО «Паляница»

## Лазерный Доктор

	name=Лазерный Доктор
	amenity=doctors
	website=https://laserdoctor.ru/
	contact:vk=https://vk.com/laserdoctor

* `website` можно уточнить на карте и в списке https://laserdoctor.ru/contacts/

## Лайм

	name=Лайм
	shop=supermarket
	website=http://limefresh.ru/
	opening_hours=24/7
	operator=ООО «Сити Ритейл»

* `website` можно уточнить в списке и на карте http://limefresh.ru/freshmarkets/

## Леди

	name=Леди
	shop=beauty
	website=https://lady-rf.ru/
	contact:vk=https://vk.com/companylady
	opening_hours=10:00-21:00

* `website` можно уточнить на карте и в списке https://lady-rf.ru/%D0%B0%D0%B4%D1%80%D0%B5%D1%81%D0%B0/

## ЛенОблФарм

	name=ЛенОблФарм
	amenity=pharmacy
	website=https://lenoblpharm.ru/
	opening_hours=09:00-21:00
	operator=ООО «Аптека «Семейная-2»

* можно проверить на карте и в списке https://lenoblpharm.ru/#map_page

## Лига Ставок

	name=Лига Ставок
	shop=bookmaker
	website=https://www.ligastavok.ru/
	contact:vk=https://vk.com/ligastavok_official

* можно проверить на карте http://corp.ligastavok.ru/Shops/sankt-peterburg/18

## Линдфорс

	name=Линдфорс
	int_name=Lindfors
	amenity=cafe
	website=http://spb.lindfors.ru/
	contact:vk=https://vk.com/cafe_lindfors

Альтернативные теги:

	opening_hours=08:00-21:00
	opening_hours=09:00-21:00
	operator=ООО «Полтава»
	operator=ООО «Столовая 41»

* `website` можно уточнить в списке http://spb.lindfors.ru/restorany
* разные операторы, у одного может быть несколько точек

## ЛюдиЛюбят

	name=ЛюдиЛюбят
	shop=bakery
	website=http://ludilove.ru/
	contact:vk=https://vk.com/ludilovebakery
	opening_hours=Mo-Fr 08:00-22:00; Sa-Su 09:00-22:00
	operator=ООО Булочная №1

* можно проверить на карте http://ludilove.ru/#map

## М65

	name=М65
	shop=outdoor
	website=http://m65-spb.ru/
	contact:vk=https://vk.com/m65_spb
	opening_hours=10:00-21:00

* можно проверить в списке http://m65-spb.ru/%D0%BD%D0%B0%D1%88%D0%B8-%D0%BC%D0%B0%D0%B3%D0%B0%D0%B7%D0%B8%D0%BD%D1%8B/

## Магнит

	name=Магнит
	brand=Магнит
	brand:wikidata=Q940518
	shop=supermarket
	website=http://magnit-info.ru/
	opening_hours=09:30-22:30
	operator=АО «Тандер»

* можно проверить на карте и в списке http://magnit-info.ru/buyers/adds/

## Макдоналдс

	name=Макдоналдс
	int_name=McDonald's
	brand=McDonald's
	brand:wikidata=Q38076
	amenity=fast_food
	website=https://mcdonalds.ru/
	contact:vk=https://vk.com/mcdonaldsrussia
	operator=ООО «Макдоналдс»

Альтернативные теги:

	amenity=cafe
	name=МакКафе
	contact:vk=https://vk.com/mccafe_russia

* `website` можно уточнить на карте https://mcdonalds.ru/restaurants/map

## Мастер кебаб

	name=Мастер кебаб
	website=http://masterkebab.ru/
	amenity=fast_food
	cuisine=kebab
	contact:vk=https://vk.com/public84455935

* можно проверить на карте http://masterkebab.ru/index2.html

## Машина времени

	name=Машина времени
	craft=watchmaker
	website=https://www.mv-time.ru/
	opening_hours=10:00-21:00

* можно проверить в списке https://www.mv-time.ru/contacts

## Мебель тут дешевле

	name=Мебель тут дешевле
	shop=furniture
	website=http://mebeltut.ru/
	contact:vk=https://vk.com/mebeltutru

* можно проверить на карте и в списке http://mebeltut.ru/about/salony/map.php/

## МегаФон

	name=МегаФон
	brand=МегаФон
	brand:wikidata=Q1720713
	shop=mobile_phone
	office=telecommunication
	website=https://spb.megafon.ru/
	contact:vk=https://vk.com/megafon
	opening_hours=09:00-21:00
	operator=АО «МегаФон Ритейл»

Альтернативные теги:

	name=МегаФон Экспресс

* можно проверить на карте https://spb.megafon.ru/help/offices/

## Меди

	name=Меди
	amenity=doctors
	website=https://medi.spb.ru/
	contact:vk=https://vk.com/mediclinics
	operator=ЗАО «Меди»

Альтернативные теги:

	amenity=dentist

	opening_hours=08:00-21:30
	opening_hours=24/7

* `website` и `name` можно уточнить на карте https://medi.spb.ru/medi/kliniki/
* страницы клиник на сайте разделены по направлениям деятельности, если у одной клиники несколько направлений, то смысла уточнять `website` может не быть

## Медок

	name=Медок
	shop=honey
	website=https://www.medokspb.ru/

* можно проверить на карте и в списке https://www.medokspb.ru/shops

## Минута Маркет

	name=Минута Маркет
	shop=convenience
	website=http://www.minutamarket.ru/
	contact:vk=https://vk.com/minuta_market

* можно проверить в списке http://www.minutamarket.ru/store/

## Мишка

	name=Мишка
	shop=bakery
	contact:vk=https://vk.com/mishka_bakery
	contact:instagram=https://www.instagram.com/mishka_bakery/
	operator=ИП Котлова Ирина Владимировна

Альтернативные теги:

	opening_hours=08:00-23:00
	opening_hours=09:00-23:00

## Многофункциональный центр

	name=Многофункциональный центр
	short_name=МФЦ
	alt_name=Мои Документы
	office=government
	website=https://gu.spb.ru/mfc/

* `website` и `name` можно уточнить в списке https://gu.spb.ru/mfc/list/
* можно проверить на карте https://моидокументы.рф/search/mfc

## Монополь

	name=Монополь
	shop=alcohol
	website=https://www.monopolshop.com/
	contact:vk=https://vk.com/monopolbar
	opening_hours=11:00-22:00
	operator=ООО «Монополь-Бар»

* `website` можно уточнить в списке https://www.monopolshop.com/restaurant/

## Море чая

	name=Море чая
	shop=tea
	website=http://morecha.ru/
	contact:vk=https://vk.com/morecha
	opening_hours=09:00-21:00

* можно проверить в списке http://morecha.ru/#adresa

## МТС

	name=МТС
	brand=МТС
	brand:wikidata=Q1368919
	shop=mobile_phone
	office=telecommunication
	website=https://spb.mts.ru/
	contact:vk=https://vk.com/mts

Альтернативные теги:

	name=Мобильные ТелеСистемы
	opening_hours=09:00-21:00
	opening_hours=09:00-22:00

* можно проверить в списке и на карте https://shop.mts.ru/stores/

## МТС Банк

	name=МТС Банк
	brand=МТС Банк
	brand:wikidata=Q1667422
	amenity=bank
	website=https://www.mtsbank.ru/
	contact:vk=https://vk.com/mtsbankru
	opening_hours=Mo-Fr 10:00-19:00

* `website` можно уточнить в списке https://www.mtsbank.ru/about/branches/st-petersburg/st-petersburg/

## Музей шоколада

	name=Музей шоколада
	shop=confectionery
	website=http://www.spb.muzeyshokolada.ru/
	opening_hours=11:00-21:00

## Мука

	name=Мука
	shop=bakery
	contact:vk=https://vk.com/myka_bakery

* можно проверить в списке контактов на https://vk.com/myka_bakery

## На Парах

	name=На Парах
	amenity=cafe
	website=https://напарах.рф/
	contact:vk=https://vk.com/naparah
	operator=ООО «НП-Садовая»

## Надежное Будущее

	name=Надежное Будущее
	amenity=credit_union
	website=https://kpk-nb.ru/
	contact:vk=https://vk.com/kpk_nb

* можно проверить на карте https://kpk-nb.ru/kontaktyi.html

## Народная 7Я семьЯ

	name=Народная 7Я семьЯ
	brand=Народная 7Я семьЯ
	brand:wikidata=Q4032436
	shop=supermarket
	website=http://www.7-ya.ru/
	contact:vk=https://vk.com/7yarus
	opening_hours=24/7
	operator=ООО «ТД Интерторг»
	operator:wikidata=Q16655827

Альтернативные теги:

	name=Большая семья

* можно проверить на карте http://www.7-ya.ru/gmaps/show_map.php?city=spb

## Наследие

	name=Наследие
	shop=gift
	website=http://www.present.ru/
	contact:vk=https://vk.com/present_ru
	opening_hours=09:00-22:00

* `website` можно уточнить в списке http://www.present.ru/salony/#yakor2

## Настроение

	name=Настроение
	shop=florist
	website=https://buket-podarki.ru/
	contact:vk=https://vk.com/nastroeniespb
	opening_hours=08:00-22:00

* можно проверить на карте и в списке https://buket-podarki.ru/main/kontakty.html

## Невис

	name=Невис
	amenity=pharmacy
	website=https://aptekanevis.ru/
	contact:vk=https://vk.com/aptekanevis

Альтернативные теги:

	operator=ООО «Аптека Дао-Фарм»
	operator=ООО «Аптеки Невис»

	opening_hours=09:00-21:00
	opening_hours=24/7

* можно проверить на карте https://aptekanevis.ru/store/

## Невская оптика

	name=Невская оптика
	shop=optician
	website=http://www.noptica.ru/
	contact:vk=https://vk.com/optisse
	opening_hours=Mo-Sa 11:00-20:00; Su 11:00-18:00
	operator=ЗАО «Невская Оптика Холдинг»

Альтернативные теги:

	name=Вижен сервис

* `website` можно уточнить в списке и на карте https://noptica.ru/saloni/

## Невские берега

	name=Невские берега
	shop=pastry
	website=http://www.cake.spb.ru/
	contact:vk=https://vk.com/nevskie.berega

Альтернативные теги:

	opening_hours=Mo-Fr 08:00-20:00; Sa-Su 10:00-20:00
	opening_hours=Mo-Fr 08:00-20:00; Sa-Su 10:00-21:00

* можно проверить на карте и в списке http://www.cake.spb.ru/location

## Нетто

	name=Нетто
	shop=supermarket
	website=http://www.netto-shops.ru/
	contact:vk=https://vk.com/netto_shops

* `website` можно уточнить в списке http://www.netto-shops.ru/store/
* сайт сдох, некоторые места заняты магазинами Интерторга

## Норман

	name=Норман
	shop=alcohol
	website=http://norman-spb.ru/
	contact:vk=https://vk.com/ts_norman
	opening_hours=11:00-22:00
	operator=ООО «Норман СПб»

* можно проверить на карте http://norman-spb.ru/locator.html

## Озерки

	name=Озерки
	amenity=pharmacy
	website=https://6030000.ru/
	contact:vk=https://vk.com/ozerkiapteka
	opening_hours=Mo-Fr 07:00-23:00; Sa-Su 08:00-22:00
	operator=ООО «Аптеки Бинко»
	operator=ООО «Эркафарм Северо-Запад»

* можно проверить на карте https://6030000.ru/map/

## Окна Века

	name=Окна Века
	website=http://oknaweka.ru/
	contact:vk=https://vk.com/okna__weka
	craft=window_construction

* можно проверить в списке http://oknaweka.ru/contacts/

## Оксфордстрит

	name=Оксфордстрит
	shop=clothes
	website=http://oxford-street.ru/
	contact:vk=https://vk.com/oxfordstreetspb
	opening_hours=10:00-21:00

* можно проверить на карте и в списке http://oxford-street.ru/contacts/

## Оливия

	name=Оливия
	shop=olive_oil
	website=http://www.paritetmm.ru/
	operator=ООО «Совместное Российско-Финское предприятие «Паритет ММ»

Альтернативные теги:

	opening_hours=Mo-Sa 10:00-20:00; Su 10:00-19:00
	opening_hours=Mo-Sa 10:00-21:00; Su 10:00-20:00

* можно проверить в списке http://www.paritetmm.ru/contacts.html

## Оранж

	name=Оранж
	shop=florist
	website=http://orangespb.ru/
	contact:vk=https://vk.com/spborange
	opening_hours=08:00-23:00

* можно проверить в списке и на карте http://orangespb.ru/shops/

## Ортека

	name=Ортека
	shop=medical_supply
	medical_supply=orthopedic
	website=https://orteka.ru/
	contact:vk=https://vk.com/orteka_ru

* `website` можно уточнить на карте https://orteka.ru/salon/

## Открытие

	name=Открытие
	brand=Открытие
	brand:wikidata=Q4327204
	amenity=bank
	website=https://www.open.ru/
	contact:vk=https://vk.com/otkritiecorp
	operator=ПАО Банк «ФК Открытие»
	operator:wikidata=Q4327204

Альтернативные теги:

	opening_hours=Mo-Sa 09:00-16:00
	opening_hours=Mo-Fr 09:00-21:00; Sa 10:00-16:00

* можно проверить на карте https://www.open.ru/addresses/map?show_atms=false

## Павловопосадские платки

	name=Павловопосадские платки
	shop=clothes
	website=http://platki.ru/
	opening_hours=Mo-Fr 11:00-20:00; Sa-Su 11:00-19:00
	operator=ООО «Платочный узор»

* можно проверить на карте http://platki.ru/stores/

## Пактор

	name=Пактор
	shop=clothes
	website=http://pactor.ru/

* `website` можно уточнить в списке http://pactor.ru/shops/

## Папироска.рф

	name=Папироска.рф
	shop=e-cigarette
	website=https://папироска.рф/
	contact:vk=https://vk.com/papiroska_rf

* `website` можно уточнить на карте и в списке https://папироска.рф/nashi-magaziny/

## Парфюмеръ

	name=Парфюмеръ
	shop=perfumery
	website=https://salonparfumer.ru/
	contact:vk=https://vk.com/salonparfumer

* `website` можно уточнить в списке https://salonparfumer.ru/salon/

## Пекарня Ф. Вольчека

см. *Булочные Ф. Вольчека*

## Пельмения

	name=Пельмения
	amenity=restaurant
	website=http://пельмения.рф/
	contact:vk=https://vk.com/club35096702
	opening_hours=11:00-23:00
	operator=ООО «Русский продукт»

* можно проверить в списке http://пельмения.рф/contacts

## Первая полоса

	name=Первая полоса
	shop=newsagent
	contact:vk=https://vk.com/club127124787

* нет сайта

## Первая помощь

	name=Первая помощь
	amenity=pharmacy
	website=http://www.aptekapp.ru/
	contact:vk=https://vk.com/apteki_raduga_pp
	opening_hours=08:00-22:00
	operator=ООО «Эркафарм Санкт-Петербург»

* `website` можно уточнить в списке http://www.aptekapp.ru/pharmacies?_filter=1&city[]=3800&web[]=69800

## Петербургские аптеки

	name=Петербургские аптеки
	amenity=pharmacy
	website=http://papteki.ru/
	operator=АО «Петербургские аптеки»

Альтернативные теги:

	opening_hours=08:00-22:00
	opening_hours=08:00-23:00

* `website` можно уточнить в списке и на карте http://papteki.ru/apteki/

## Петербургский стиль

	name=Петербургский стиль
	shop=clothes
	website=http://peterstyle.ru/
	contact:vk=https://vk.com/peterstylespb
	operator=ООО «Петербургский стиль»

* можно проверить в списке http://peterstyle.ru/ru/content/14-gde-kupit

## Петроокна

	name=Петроокна
	craft=window_construction
	website=http://petrookna.ru/
	contact:vk=https://vk.com/zavodpetrookna
	opening_hours=Mo-Sa 10:00-20:00; Su 11:00-20:00

* можно проверить в списке и на карте http://petrookna.ru/contacts

## Петроторг

	name=Петроторг
	shop=variety_store
	website=http://www.петроторг-атк.рф/
	contact:vk=https://vk.com/atk_petrotorg

* можно проверить в списке и на карте http://www.петроторг-атк.рф/shops/

## ПиLки

	name=ПиLки
	alt_name=Пилки
	shop=beauty
	website=https://pilkinail.ru/
	contact:vk=https://vk.com/pilkinail
	opening_hours=09:00-23:00

Альтернативные теги:

	operator=ИП Сенин Владимир Юрьевич
	operator=ИП Сенина Н.В.

* можно проверить в списке https://pilkinail.ru/studio?list
* можно проверить на карте https://pilkinail.ru/studio?map

## Пингвин

	name=Пингвин
	shop=dry_cleaning
	website=http://химчисткапингвин.рф/
	contact:vk=https://vk.com/club11111586

* можно проверить на карте и в списке http://химчисткапингвин.рф/#tabs_container_map

## Планета здоровья

	name=Планета здоровья
	amenity=pharmacy
	website=http://planetazdorovo.ru/
	opening_hours=24/7
	operator=ООО «Парус»

* можно проверить на карте и в списке http://planetazdorovo.ru/drugstore.aspx?Id_page=11

## Планета Секонд Хенд

	name=Планета Секонд Хенд
	shop=clothes
	website=http://planetashspb.ru/
	contact:vk=https://vk.com/mirsh
	opening_hours=10:00-21:00
	operator=ООО «Логистика СПб»

* можно проверить в списке http://planetashspb.ru/magaziny
* есть всероссийские сайты http://planeta-sh.ru/ и https://vk.com/planeta_sx

## Платина Кострома

	name=Платина Кострома
	shop=jewelry
	website=https://platina-kostroma.com/
	contact:vk=https://vk.com/platina_kostroma
	opening_hours=10:00-21:00

* можно проверить в списке https://platina-kostroma.shop/shop-peterburg

## Платформа

	name=Платформа
	shop=mall
	website=http://platforma-tk.ru/

* `website` можно уточнить на карте и в списке http://platforma-tk.ru/kontaktyi.html

## Подружка

	name=Подружка
	shop=cosmetics
	website=https://www.podrygka.ru/
	contact:vk=https://vk.com/podrygkashop
	opening_hours=09:00-21:00

* `website` можно уточнить в списке https://www.podrygka.ru/shoplist/?arrShopsSmartFilter_68=450215437&arrShopsSmartFilter_68=450215437&arrShopsSmartFilter_31=0&arrShopsSmartFilter_101=0&arrShopsSmartFilter_71=0&arrShopsSmartFilter_37=&arrShopsSmartFilter_36=0&set_filter=y

## Полушка

	name=Полушка
	brand=Полушка
	brand:wikidata=Q4371080
	shop=supermarket
	website=http://www.polushka.info/
	opening_hours=24/7

Альтернативные теги:

	operator=ООО «Любавушка Ритейл Груп»
	operator=ООО «Сити Ритейл»

* `website` можно уточнить в списке http://www.polushka.info/spb-and-len-obl

## Пончкофф

	name=Пончкофф
	amenity=cafe
	website=https://www.ponchkoff.ru/
	contact:vk=https://vk.com/ponchkoff
	opening_hours=Mo-Fr 09:00-21:00; Sa-Su 09:00-22:00
	operator=ООО «Ганеша»

* можно проверить в списке https://www.ponchkoff.ru/shops

## Пхали-Хинкали

	name=Пхали-Хинкали
	amenity=restaurant
	cuisine=georgian
	website=http://phali-hinkali.ru/
	contact:vk=https://vk.com/phalihinkali

* `website` можно уточнить

## Пышечная фабрика

	amenity=cafe
	name=Пышечная фабрика
	website=http://pishechnaya-fabrika.com/
	opening_hours=08:00-22:00
	operator=ООО «Арсенал Инвест»

* можно проверить в списке http://pishechnaya-fabrika.com/contacts/

## Пятёрочка

	name=Пятёрочка
	brand=Пятёрочка
	brand:wikidata=Q1768969
	shop=supermarket
	website=https://5ka.ru/
	contact:vk=https://vk.com/pyaterochka_shop
	operator=ООО «Агроторг»

Альтернативные теги:

	opening_hours=00:00-23:45
	opening_hours=08:00-23:00
	opening_hours=09:00-23:00
	opening_hours=24/7
	opening_hours=Mo-Fr 08:00-23:00; Sa-Su 09:00-23:00

* можно проверить на карте https://5ka.ru/stores/

## Радуга

	name=Радуга
	amenity=pharmacy
	website=http://apteka-raduga.ru/
	contact:vk=https://vk.com/apteki_raduga_pp

Альтернативные теги:

	operator=ООО «Роста»
	operator=ООО «Эркафарм Санкт-Петербург»

* можно проверить в списке http://apteka-raduga.ru/addresses/spb/

## Райффайзенбанк

	name=Райффайзенбанк
	brand:wikidata=Q2127528
	brand=Raiffeisen
	amenity=bank
	website=https://www.raiffeisen.ru/
	contact:vk=https://vk.com/raiffeisenbankrus
	opening_hours=Mo-Fr 11:00-20:00; Sa 11:00-17:00
	operator=АО «Райффайзенбанк»
	operator:wikidata=Q4389244

* `website` можно уточнить на карте https://www.raiffeisen.ru/offices/

## РеалЪ

	name=РеалЪ
	shop=supermarket
	website=http://www.tdreal.spb.ru/
	contact:vk=https://vk.com/real_svezhie_produkty
	operator=ООО «Торговый дом «Реалъ»

Альтернативные теги:

	opening_hours=09:00-23:00
	opening_hours=24/7

* `website` можно уточнить в списке http://www.tdreal.spb.ru/

## Ренессанс Кредит

	name=Ренессанс Кредит
	brand=Ренессанс Кредит
	brand:wikidata=Q16691879
	amenity=bank
	website=https://rencredit.ru/
	contact:vk=https://vk.com/renaissancecredit
	operator=Коммерческий банк «Ренессанс Кредит» (ООО)
	operator:wikidata=Q16691879

* можно проверить на карте https://rencredit.ru/addresses/

## РЕСО-Гарантия

	name=РЕСО-Гарантия
	brand=РЕСО-Гарантия
	brand:wikidata=Q4386516
	office=insurance
	website=http://www.reso.ru/
	contact:vk=https://vk.com/reso_garantia
	opening_hours=Mo-Fr 10:00-19:00; Sa 10:00-18:00; Su 10:00-16:00

* можно проверить на карте http://www.reso.ru/About/Contacts/

## РЕСО-Мед

	name=РЕСО-Мед
	office=insurance
	website=https://spb.reso-med.com/
	contact:vk=https://vk.com/resomed
	opening_hours=Mo-Fr 09:30-18:00
	operator=ООО «СМК РЕСО-Мед»

* можно проверить на карте https://spb.reso-med.com/police_omc/where_get_policy_omc/

## Рив Гош

	name=Рив Гош
	brand=Рив Гош
	brand:wikidata=Q4394330
	shop=cosmetics
	website=http://www.rivegauche.ru/
	contact:vk=https://vk.com/rivegauche
	operator=ООО «АромаЛюкс»

Альтернативные теги:

	opening_hours=10:00-21:00
	opening_hours=Su-Th 10:00-21:00; Fr-Sa 10:00-22:00

* `website` можно уточнить на карте и в списке http://www.rivegauche.ru/shops/cities/sankt-peterburg

## РосАл

	name=РосАл
	shop=alcohol
	website=http://www.rosal24.ru/
	opening_hours=11:00-22:00
	operator=ООО «Актив 78»

Альтернативные теги:

	opening_hours=11:00-22:00 open "магазин и бар", 22:00-11:00 open "бар"

* одна из сетей магазинов алкоголя, которые ночью работали [типа как бары](https://www.fontanka.ru/2017/10/18/042/)
* некоторые из них, возможно, и сейчас так работают, во всяком случае, на них до сих пор это написано

## Росбанк

	name=Росбанк
	brand=Росбанк
	brand:wikidata=Q1119857
	amenity=bank
	website=https://www.rosbank.ru/
	contact:vk=https://vk.com/rosbank
	opening_hours=Mo-Fr 09:00-20:00; Sa 10:00-17:00

* `website` можно уточнить на карте https://www.rosbank.ru/ru/offices/
* `website` можно уточнить в списке https://www.rosbank.ru/ru/offices/list.php

## Росгосстрах

	name=Росгосстрах
	brand=Росгосстрах
	brand:wikidata=Q4397745
	office=insurance
	website=https://www.rgs.ru/
	contact:vk=https://vk.com/rosgosstrakh_official
	opening_hours=Mo-Fr 09:00-20:00; Sa-Su 10:00-18:00

* `website` можно уточнить на карте и в списке https://www.rgs.ru/contacts/list.wbp#id=2709

## Росгосстрах Банк

	name=Росгосстрах Банк
	brand=Росгосстрах Банк
	brand:wikidata=Q4397742
	amenity=bank
	website=https://www.rgsbank.ru/
	contact:vk=https://vk.com/rgsbank
	opening_hours=Mo-Fr 09:00-20:00; Sa-Su 10:00-18:00
	operator=ПАО «Росгосстрах Банк»
	operator:wikidata=Q4397742

* `website` можно уточнить на карте и в списке https://www.rgsbank.ru/about/branches_and_atms/

## Роскошь золота

	name=Роскошь золота
	shop=jewelry
	website=http://rzol.ru/
	contact:vk=https://vk.com/roskosh_zolota
	opening_hours=10:00-21:00

* можно проверить в списке и на карте http://rzol.ru/contacts/

## Роспечать

	name=Роспечать
	brand=Роспечать
	brand:wikidata=Q4397882
	shop=newsagent
	website=http://rosp.ru/

## Российский Красный Крест

	name=Российский Красный Крест
	office=ngo
	website=http://www.spbredcross.org/
	contact:vk=https://vk.com/redcrossspb

* можно проверить на карте http://www.spbredcross.org/about/contacts

## Россия

	name=Россия
	brand=Россия
	brand:wikidata=Q827011
	amenity=bank
	website=http://abr.ru/

* можно проверить на карте http://abr.ru/about/contacts/#map-officies

## Санкт-Петербург

	name=Санкт-Петербург
	brand=Банк Санкт-Петербург
	brand:wikidata=Q4407652
	amenity=bank
	website=https://www.bspb.ru/
	contact:vk=https://vk.com/bspb
	operator=ПАО «Банк «Санкт-Петербург»
	operator:wikidata=Q4407652

## Санкт-Петербургский городской ломбард

	name=Санкт-Петербургский городской ломбард
	shop=pawnbroker
	website=http://www.lombard.sp.ru/
	opening_hours=Mo-Sa 10:00-20:00; Su 10:00-18:00
	operator=ООО «Санкт-Петербургский городской ломбард»

* можно проверить на карте и в списке http://www.lombard.sp.ru/lombard_new.nsf/(sitetree)/AF9568222E328185C32573D30027556F?OpenDocument

## СантаМаг

	name=СантаМаг
	shop=supermarket
	website=http://santa-mag.ru/
	contact:vk=https://vk.com/club144216822
	opening_hours=24/7

* можно проверить на карте http://santa-mag.ru/magaziny/

## Сбербанк

	name=Сбербанк
	brand=Сбербанк
	brand:wikidata=Q205012
	amenity=bank
	website=https://www.sberbank.ru/
	contact:vk=https://vk.com/sberbank
	operator=ПАО «Сбербанк России»
	operator:wikidata=Q205012

* можно проверить на карте https://www.sberbank.ru/ru/about/today/oib
* можно проверить на карте (для бизнеса) https://www.sberbank.ru/ru/about/today/oib?legal_person=1

## Связной

	name=Связной
	brand=Связной
	brand:wikidata=Q65371
	shop=mobile_phone
	website=https://www.svyaznoy.ru/
	contact:vk=https://vk.com/svyaznoy
	opening_hours=09:00-22:00

* можно проверить на карте https://www.svyaznoy.ru/shops

## СДЭК

	name=СДЭК
	brand=СДЭК
	brand:wikidata=Q28665980
	amenity=post_office
	website=https://www.cdek.ru/
	contact:vk=https://vk.com/cdek_express
	opening_hours=Mo-Fr 10:00-20:00; Sa 10:00-16:00; Su 10:00-14:00
	operator=ИП Трохов Кирилл Анатольевич

* `website` можно уточнить в списке и на карте https://www.cdek.ru/contacts/sankt-peterburg.html

## Север-Метрополь

	name=Север-Метрополь
	amenity=cafe
	shop=pastry
	website=http://sever.spb.su/
	contact:vk=https://vk.com/sever__metropol

Альтернативные теги:

	opening_hours=09:00-21:00
	operator=ИП Хилимон Зинаида Васильевна

* `website` можно уточнить в списке http://sever.spb.su/wheretobuy/
* `website` можно уточнить на карте http://sever.spb.su/wheretobuy/?city_map
* на Невском 44 находится собственно *Север*
* на Садовой 22 находится собственно *Метрополь*
* также есть сайт http://metropol.spb.su/
* также есть редирект с http://tort.spb.ru/
* разные операторы, часто ИП, но даже у ИП может быть несколько точек

## Семишагофф

	name=Семишагофф
	website=https://semishagoff.org/
	contact:vk=https://vk.com/7shagoff
	shop=supermarket

Альтернативные теги:

	opening_hours=09:00-21:00
	opening_hours=09:00-23:00

	operator=ООО «Витан»
	operator=ООО «ТК Прогресс»
	operator=ООО «ТК Триумф»

* можно проверить в меню *Адреса магазинов* на https://vk.com/7shagoff

## Сенатор

	name=Бизнес-центр «Сенатор»
	landuse=commercial
	website=http://senator.spb.ru/
	contact:vk=https://vk.com/bcsenator

* `website` можно уточнить

## Серебро

	name=Серебро
	shop=jewelry
	website=http://serebro-juvelir.com/

* можно проверить в списке http://serebro-juvelir.com/shops

## Синнабон

	name=Синнабон
	int_name=Cinnabon
	brand=Cinnabon
	brand:wikidata=Q1092539
	amenity=cafe
	website=https://cinnabonrussia.com/
	contact:vk=https://vk.com/spbcinnabon
	opening_hours=Mo-Th 08:00-22:30; Fr 08:00-23:00; Sa-Su 09:00-23:00

* можно проверить в списке https://vk.com/spbcinnabon
* можно проверить в списке и на карте https://cinnabonrussia.com/locations/?location=%D1%81%D0%B0%D0%BD%D0%BA%D1%82-%D0%BF%D0%B5%D1%82%D0%B5%D1%80%D0%B1%D1%83%D1%80%D0%B3

## Ситилинк

	name=Ситилинк
	brand=Ситилинк
	brand:wikidata=Q16698091
	shop=electronics
	website=https://www.citilink.ru/
	contact:vk=https://vk.com/citilink_ru
	opening_hours=10:00-21:00
	operator=ООО «Ситилинк»

Альтернативные теги:

	name=Ситилинк-мини

* `website` можно уточнить в списке и на карте https://www.citilink.ru/about/stores/

## Скарлетт

	name=Скарлетт
	shop=chemist
	website=http://scarlett-group.ru/
	opening_hours=09:00-22:00

* можно проверить в списке и на карте http://scarlett-group.ru/adresa-magazinov.php

## Сладкоежка

	name=Сладкоежка
	amenity=cafe
	website=https://www.sladkoezka.com.ru/

Альтернативные теги:

	opening_hours=Mo-Fr 08:00-21:00; Sa-Su 09:00-21:00
	opening_hours=Mo-Fr 08:00-22:00; Sa-Su 09:00-22:00

	operator=ООО «Апельсин»
	operator=ООО «Тим»

* можно проверить на карте и в списке https://www.sladkoezka.com.ru/restorany/kontakty
* https://vk.com/tortsladkoezka не подходит, так как относится к кондитерской фабрике
* разные операторы

## Слетать.ру

	name=Слетать.ру
	shop=travel_agency
	website=https://sletat.ru/
	contact:vk=https://vk.com/sletat_ru

* можно проверить на карте https://sletat.ru/#offices
* `website` можно уточнить
* `contact:vk` можно уточнить

## Снаряжение

	name=Снаряжение
	shop=outdoor
	website=http://www.equip.ru/
	contact:vk=https://vk.com/equipru
	opening_hours=Mo-Sa 10:00-20:00; Su 10:00-18:00
	operator=ООО «Вектор»

* `website` можно уточнить в списке http://www.equip.ru/gde_kupit7

## Совкомбанк

	name=Совкомбанк
	brand=Совкомбанк
	brand:wikidata=Q4426566
	amenity=bank
	website=https://sovcombank.ru/
	opening_hours=Mo-Fr 10:00-20:00; Sa 10:00-17:00

* можно проверить в списке https://sovcombank.ru/office-atm/branches/list/leningradskaya-oblast/

## Солнцетур

	name=Солнцетур
	shop=travel_agency
	website=https://solncetur.ru/
	contact:vk=https://vk.com/solncetur

* можно проверить на карте и в списке https://solncetur.ru/contacts

## Соната

	name=Соната
	int_name=Sonata
	tourism=hotel
	website=http://hotel-sonata.ru/
	contact:vk=https://vk.com/sonata_hotels_group

* можно проверить в списке http://hotel-sonata.ru/raspolozhenie-.html
* `name` и `website` можно уточнить у некоторых точек

## Спектр

см. *Watsons*

## Старбакс

	name=Старбакс
	int_name=Starbucks
	brand=Starbucks
	brand:wikidata=Q37158
	amenity=cafe
	website=http://www.starbucks.ru/
	contact:vk=https://vk.com/starbucks
	opening_hours=7:30-23:00

* `website` можно уточнить на карте http://www.starbucks.ru/store-locator/search/location/190000

## Стильное серебро

	name=Стильное серебро
	shop=jewelry
	website=https://www.stilnoeserebro.ru/
	contact:vk=https://vk.com/stilnoeserebro
	operator=ИП Поленов Г.А.

Альтернативные теги:

	opening_hours=Mo-Fr 10:00-21:00; Sa-Su 11:00-20:00
	opening_hours=Mo-Fr 10:00-21:00; Sa-Su 11:00-21:00

* можно проверить в списке https://www.stilnoeserebro.ru/kontakty

## Столовая №1

	name=Столовая №1
	amenity=fast_food
	fast_food=cafeteria
	website=https://st1.one/
	contact:vk=https://vk.com/club12879930
	opening_hours=24/7

Альтернативные теги:

	operator=ООО «Большая Медведица»
	operator=ООО «Гуд Фуд Некрасова»

* можно проверить в списке и на карте https://st1.one/#contacts
* разные операторы

## Стома

	name=Стома
	amenity=dentist
	website=http://ctoma.ru/
	contact:vk=https://vk.com/ctoma
	opening_hours=09:00-21:00

* `website` можно уточнить на карте http://ctoma.ru/clinics

## СуперХостел

	name=СуперХостел
	int_name=SuperHostel
	tourism=hostel
	website=https://superhostel.ru/
	contact:vk=https://vk.com/superhostel

* `website` можно уточнить в списке и на карте https://superhostel.ru/

## Суши Wok

	name=Суши Wok
	amenity=fast_food
	cuisine=sushi
	website=https://sushiwok.ru/spb/
	contact:vk=https://vk.com/woksushi

* можно проверить в списке https://sushiwok.ru/spb/addresses/

## СушиШоп

	name=СушиШоп
	amenity=fast_food
	cuisine=sushi
	contact:vk=https://vk.com/sushishop
	website=https://www.sushishop.ru/
	opening_hours=10:00-23:00

* можно проверить на карте http://www.sushishop.ru/magaziny/

## Счастливый взгляд

	name=Счастливый взгляд
	shop=optician
	website=https://happylook.ru/
	contact:vk=https://vk.com/happylook_ru
	opening_hours=10:00-20:00
	operator=ООО «Оптик Комфорт»

* `website` можно уточнить на карте и в списке https://happylook.ru/stores/spb/

## Табак Sity

	name=Табак Sity
	alt_name=Табак Сити
	shop=tobacco
	website=http://tabaksity.ru/
	opening_hours=08:00-23:00

* сайт не работает

## Табакерка

	name=Табакерка
	shop=tobacco
	website=http://www.tabakerka.biz/
	opening_hours=10:00-21:00
	operator=ООО «Омега»

* `website` можно уточнить

## Тан Жен

	name=Тан Жен
	amenity=restaurant
	cuisine=chinese
	website=http://www.tang-ren.ru/
	contact:vk=https://vk.com/tangrenspb
	opening_hours=12:00-23:00

Альтернативные теги:

	operator=ООО «Родос»
	operator=ООО «Тан Жен»

* `website` можно уточнить в списке http://www.tang-ren.ru/rest.htm
* у них на карте точек меньше, чем в списке

## Тарелка столовая

	name=Тарелка столовая
	amenity=fast_food
	fast_food=cafeteria
	website=http://tarelkas.ru/
	contact:vk=https://vk.com/tarelka_st

Альтернативные теги:

	opening_hours=08:00-23:00
	opening_hours=Su-We 07:30-23:00; Th-Sa 00:00-24:00

	operator=ООО «Арес»
	operator=ООО «Прогресс+»

* `website` можно уточнить в списке http://tarelkas.ru/cafes/
* `website` можно уточнить на карте http://tarelkas.ru/cafes/map/

## Твои деньги

	name=Твои деньги
	shop=money_lender
	website=http://www.tvoidengi.com/

* можно проверить в списке http://www.tvoidengi.com/office/

## Теремок

	name=Теремок
	brand=Теремок
	brand:wikidata=Q4455593
	amenity=fast_food
	cuisine=pancake
	website=http://www.teremok.ru/
	contact:vk=https://vk.com/teremok
	operator=ООО «Теремок — Русские Блины»

Альтернативные теги:

	opening_hours=07:30-23:00
	opening_hours=Mo-Fr 07:30-23:00; Sa-Su,PH 08:30-23:00
	opening_hours=08:30-23:00

* `website` можно уточнить на карте и в списке http://www.teremok.ru/address/

## Террапевтика

	name=Террапевтика
	shop=medical_supply
	medical_supply=orthopedic
	website=https://www.terrapevtika.ru/
	contact:vk=https://vk.com/terrapevtika
	opening_hours=Mo-Sa 09:00-20:00; Su 10:00-18:00
	operator=ИП Гусаров А.М.

* `website` можно уточнить на карте и в списке https://www.terrapevtika.ru/adresa-magazinov/

## Токио-City

	name=Токио-City
	name:ru=Токио-Сити
	name:en=Tokyo-City
	amenity=restaurant
	website=https://www.tokyo-city.ru/
	contact:vk=https://vk.com/club_tokyocity
	opening_hours=Mo-Fr 11:00-23:00; Sa-Su 12:00-23:00

Альтернативные теги:

	operator=ООО «Армалит»
	operator=ООО «Нептун»

* можно проверить на карте и в списке https://www.tokyo-city.ru/adresses.html

## Толстый фраер

	name=Толстый фраер
	amenity=pub
	website=http://tolstiy-fraer.ru/
	contact:vk=https://vk.com/tolstiyfraer
	opening_hours=Su-Th 10:00-01:00, Fr-Sa 10:00-03:00

Альтернативные теги:

	operator=ООО «Толстый фраер - 3»
	operator=ООО «Флай»

* `website` можно уточнить

## Точка зрения

	name=Точка зрения
	shop=optician
	website=http://www.optica-tz.ru/
	opening_hours=10:00-22:00

* можно проверить на карте и в списке http://www.optica-tz.ru/shops/

## ТриКотаЖ

	name=ТриКотаЖ
	shop=clothes
	website=https://www.trikstore.ru/
	contact:vk=https://vk.com/trikstore
	opening_hours=10:00-20:00
	operator=ООО «Трик»

* можно проверить в списке https://www.trikstore.ru/shops/

## Удачная

	name=Удачная
	amenity=pharmacy
	opening_hours=09:00-21:00
	operator=ООО «Фарммедсервис»

## Улыбка радуги

	name=Улыбка радуги
	shop=chemist
	website=https://www.r-ulybka.ru/
	contact:vk=https://vk.com/ulybka_radugi
	opening_hours=09:00-22:00

Альтернативные теги:

	operator=ООО «Петроградское»
	operator=ООО «Южное»

* можно проверить на карте https://www.r-ulybka.ru/u-store/

## Университетская аптека

	name=Университетская аптека
	brand=Yliopiston Apteekki
	brand:wikidata=Q10709719
	amenity=pharmacy
	website=http://yapteka.ru/
	contact:vk=https://vk.com/yapteka
	opening_hours=Mo-Fr 09:00-21:00; Sa-Su 10:00-21:00

* `website` можно уточнить на карте и в списке http://yapteka.ru/company/shops/

## Унция

	name=Унция
	shop=tea
	website=https://www.chay.info/
	contact:vk=https://vk.com/teaounce

Альтернативные теги:

	shop=coffee

	opening_hours=09:30-21:00
	opening_hours=10:00-22:00

* можно проверить в списке https://www.chay.info/store/sanktpeterburg/

## Уральский банк реконструкции и развития

	name=Уральский банк реконструкции и развития
	brand=Уральский банк реконструкции и развития
	brand:wikidata=Q4476745
	amenity=bank
	website=https://spb.ubrr.ru/
	contact:vk=https://vk.com/bank_ubrr

* можно проверить на карте https://spb.ubrr.ru/ofisy-i-bankomaty

## Фаворит

	name=Фаворит
	shop=shoes
	website=http://obuv-favorit.ru/
	opening_hours=10:00-21:00

* можно проверить в списке http://obuv-favorit.ru/adresa_magazinov
* `website` можно было бы уточнить, но страницы магазинов почти пустые

## Фантазия

	name=Фантазия
	shop=florist
	website=https://eliteflower.ru/
	contact:vk=https://vk.com/eliteflower
	operator=ООО «Фантазия-4»

Альтернативные теги:

	opening_hours=08:00-22:00
	opening_hours=24/7

* можно проверить на карте https://eliteflower.ru/kontakty/

## Фара

	name=Фара
	amenity=driving_school
	website=http://auto-fara.ru/
	contact:vk=https://vk.com/auto_fara

* `website` можно уточнить

## Фиалка

	name=Фиалка
	amenity=pharmacy
	website=https://fialkaspb.ru/
	contact:vk=https://vk.com/aptekafialka
	opening_hours=24/7

* `website` можно уточнить на карте https://fialkaspb.ru/pharmacy/

## Филя

	name=Филя
	shop=pet
	website=https://www.zootovar-spb.ru/
	contact:vk=https://vk.com/zootovarfilya
	opening_hours=10:00-21:00
	operator=ИП Дреничев А.А.

* `website` можно уточнить на карте и в списке https://www.zootovar-spb.ru/page_31.html

## Фруктовая лавка

	name=Фруктовая лавка
	shop=greengrocer
	website=http://fruktlove.ru/
	contact:vk=https://vk.com/club38322205
	opening_hours=10:00-22:00

* `website` можно уточнить
* есть магазин, совмещённый с кафе

## Харбин

	name=Харбин
	amenity=restaurant
	cuisine=chinese
	website=https://harbin-spb.ru/
	opening_hours=12:00-23:00
	operator=ООО «Хондэ»

* `website` можно уточнить в списке https://harbin-spb.ru/kontakty/

## Хачапури и вино

	name=Хачапури и вино
	amenity=restaurant
	cuisine=georgian
	website=http://hachapuriivino.ru/
	contact:vk=https://vk.com/hachapuriivino
	opening_hours=09:00-03:00

* можно проверить в списке http://hachapuriivino.ru/index.php?route=information/information&information_id=7

## Хесбургер

	name=Хесбургер
	int_name=Hesburger
	brand=Hesburger
	brand:wikidata=Q1276832
	amenity=fast_food
	website=https://www.hesburger.ru/
	contact:vk=https://vk.com/hesburger_russia

* `website` можно уточнить на карте https://www.hesburger.ru/260/?country=ru&city=%D1%81%D0%B0%D0%BD%D0%BA%D1%82-%D0%BF%D0%B5%D1%82%D0%B5%D1%80%D0%B1%D1%83%D1%80%D0%B3

## Хорошая связь

	name=Хорошая связь
	shop=mobile_phone
	website=https://goodcom.ru/
	contact:vk=https://vk.com/goodcom_pro

* можно проверить в списке и на карте https://goodcom.ru/contacts

## Ц!Обувь

	name=Ц!Обувь
	shop=shoes
	website=http://soglasie77.ru/
	contact:vk=https://vk.com/shoesfromyou
	opening_hours=10:00-21:00

## Цветовик

	name=Цветовик
	shop=florist
	website=https://cvetovik.com/
	contact:vk=https://vk.com/cvetovikspb
	opening_hours=24/7

* можно проверить на карте https://cvetovik.com/contacts/

## Цветомания

	name=Цветомания
	shop=florist
	website=https://tsvetomania.ru/
	contact:vk=https://vk.com/tsvetomania
	opening_hours=10:00-19:00
	operator=ООО «Цветомания»

* можно проверить в списке и на карте https://tsvetomania.ru/contacts.php

## Цветочный блюз

	name=Цветочный блюз
	shop=florist
	contact:vk=https://vk.com/fantasysp
	opening_hours=24/7
	operator=ИП "Погорелова И.В."

## Центр Займов

	name=Центр Займов
	shop=money_lender
	website=https://centrzaimov.ru/
	contact:vk=https://vk.com/centrzaimov

* `website` можно уточнить в меню *Офисы*

## Цех 85

	name=Цех 85
	amenity=cafe
	shop=bakery
	website=http://tseh85.ru/
	contact:vk=https://vk.com/tseh85

Альтернативные теги:

	opening_hours=Mo-Fr 08:00-22:00; Sa-Su 09:00-22:00
	opening_hours=Mo-Fr 08:00-22:00; Sa-Su 10:00-22:00

* можно проверить в списке и на карте http://tseh85.ru/#contact-section
* надо проверять, чем в большей степени является amenity=cafe или shop=bakery

## Чайникофф

	name=Чайникофф
	amenity=cafe
	website=http://www.chainikoff.ru/
	contact:vk=https://vk.com/chainikoff_cafe
	opening_hours=24/7
	operator=ООО «Кофейня 24»

* можно проверить на карте и в списке http://www.chainikoff.ru/coffeemap/sankt-peterburg.html

## Чайный остров

	name=Чайный остров
	shop=tea
	website=http://tea-ostrov.ru/
	opening_hours=Mo-Sa 09:00-21:00; Su 10:00-19:00
	operator=ИП Краснова Ольга Михайловна

* можно проверить в списке и на карте http://tea-ostrov.ru/#spb

## Чебуroom

	name=Чебуroom
	amenity=cafe
	website=https://cheburoom.ru/
	contact:vk=https://vk.com/cheburoom

* можно проверить в списке и на карте https://cheburoom.ru/#b6

## Четыре глаза

	name=Четыре глаза
	shop=optics
	website=https://www.4glaza.ru/
	contact:vk=https://vk.com/chetyreglaza
	opening_hours=10:00-21:00
	operator=ООО «Четыре глаза»

* можно проверить в списке и на карте https://www.4glaza.ru/shops/sankt-peterburg/

## Шоколадница

	name=Шоколадница
	brand=Шоколадница
	brand:wikidata=Q30891188
	amenity=cafe
	cuisine=coffee_shop
	website=http://shoko.ru/sankt-peterburg/
	contact:vk=https://vk.com/shokospb

Альтернативные теги:

	opening_hours=08:00-23:00
	opening_hours=24/7
	operator=ООО «Галерея-Алекс»
	operator=ООО «Кофе Хаус. Эспрессо и Капучино Бар»

* можно проверить на карте http://shoko.ru/sankt-peterburg/adresa_kofeen/
* разные операторы
* некоторые из точек - бывшие "Кофе Хаузы"

## Штолле

	name=Штолле
	amenity=cafe
	website=https://spb.stolle.ru/
	contact:vk=https://vk.com/stollerussia

Альтернативные теги:

	opening_hours=Mo-Fr 08:00-21:00; Sa-Su 09:00-21:00
	opening_hours=09:00-21:00

	operator=ООО «Столовая №55»
	operator=ООО «Столовая №63»

* `website` можно уточнить на карте и в списке https://spb.stolle.ru/addresses

## ЭнергоМашБанк

	name=ЭнергоМашБанк
	brand=Энергомашбанк
	brand:wikidata=Q4531968
	amenity=bank
	website=https://www.energomashbank.ru/
	contact:vk=https://vk.com/embclub

* `website` можно уточнить на карте и в списке https://www.energomashbank.ru/main/bank/offices

## Эталон ЛенСпецСМУ

	name=Эталон ЛенСпецСМУ
	brand=Эталон
	brand:wikidata=Q4532935
	office=estate_agent
	website=http://lenspecsmu.ru/
	contact:vk=https://vk.com/gk_etalon
	operator=АО «Эталон ЛенСпецСМУ»

* можно проверить на карте и в списке http://lenspecsmu.ru/contacts/spb/

## Ювелирторг

	name=Ювелирторг
	brand=Ювелирторг
	shop=jewelry
	website=http://www.juvelirtorg.spb.ru/
	contact:vk=https://vk.com/juvelirtorg
	opening_hours=Mo-Sa 10:00-21:00; Su 11:00-21:00
	operator=АО «Ювелирторг»

* `name` и `website` можно уточнить на карте и в списке http://www.juvelirtorg.spb.ru/stores

## Юлмарт

	name=Юлмарт Outpost
	brand=Юлмарт
	brand:wikidata=Q4534379
	shop=electronics
	website=https://www.ulmart.ru/
	contact:vk=https://vk.com/ulmart
	opening_hours=Mo-Fr 10:00-22:00; Sa-Su 10:00-21:00; 13:30-14:00 off
	operator=ООО «Юлмарт РСК»

* `website` можно уточнить в списке https://www.ulmart.ru/help/spb/contacts

## ЮниКредит Банк

	name=ЮниКредит Банк
	brand=UniCredit
	brand:wikidata=Q45568
	amenity=bank
	website=https://www.unicreditbank.ru/
	contact:vk=https://vk.com/unicreditbank
	operator=АО «ЮниКредит Банк»
	operator:wikidata=Q727011

Альтернативные теги:

	opening_hours=Mo-Th 09:00-20:00; Fr 09:00-19:00; Sa 10:00-17:00

* можно проверить на карте https://www.unicreditbank.ru/ru/branch-finder.html

## Юнистрим

	name=Юнистрим
	brand=Юнистрим
	brand:wikidata=Q4534572
	amenity=bank
	website=http://unistream.ru/
	contact:vk=https://vk.com/unistreambank
	opening_hours=09:30-20:30

* можно проверить на карте http://unistream.ru/service-points/

## Юниум

	name=Юниум
	amenity=training
	description=курсы для школьников
	website=https://unium.ru/
	contact:vk=https://vk.com/spb_unium

* `website` можно уточнить на карте https://unium.ru/contacts/

## Я — Актер!

	name=Я — Актер!
	amenity=training
	description=актёрское мастерство для детей
	website=http://i-actor.ru/
	contact:vk=https://vk.com/iactorspb

* можно проверить в списке http://i-actor.ru/contacts/

## Япоша

	name=Япоша
	amenity=restaurant
	website=http://www.yaposhaspb.com/
	contact:vk=https://vk.com/yaposhasushi

Альтернативные теги:

	opening_hours=11:00-23:00
	opening_hours=Mo-Fr 09:00-23:00; Sa-Su 11:00-23:00

	operator=ООО «А8-Итальянская»
	operator=ООО «Орион»

* можно проверить на карте и в списке http://www.yaposhaspb.com/address.html
* разные операторы

## Яркий

	name=Яркий
	shop=photo
	website=https://www.yarkiy.ru/
	contact:vk=https://vk.com/yarkiy
	opening_hours=09:00-21:00

`website` можно уточнить на карте и в списке https://www.yarkiy.ru/stores

## Яркий мир рукоделия

	name=Яркий мир рукоделия
	shop=sewing
	website=http://mir-ruk.ru/
	contact:vk=https://vk.com/mir_rukodelija

* можно проверить в списке http://mir-ruk.ru/store/
